...
> = (set, get) => ({
  ...
  selectConversation: async (id: string) => {
    try {
      ...
      set({ error: "Geçmiş Load Failed" });
    }
  },

  startNewChat: async (title: string = "New chat") => {
    try {
      const { id } = await createConversation(title, "code");
      await get().loadConversations();
      await get().selectConversation(id);
    } catch {
      set({ error: "Failed to start new chat" });
    }
  },
  ...
  sendMessage: async (content: string, model: string = "gemini-1.5-pro") => {
    ...
    if (!convId) {
      try {
        ...
        set({ error: "Failed to start chat" });
        return;
      }
    }
