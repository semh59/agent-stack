...
# Allow execution role to read the secrets we wire into the tasks.
...
# Listener is HTTP-only here; in a real deploy attach an ACM cert and switch
# to HTTPS. Kept as HTTP so `terraform validate` doesn't need a cert ARN.
...
module "gateway" {
  ...
  environment = {
    ...
    ALLOY_BRIDGE_HOST  = "optimization-bridge.prod.internal"
