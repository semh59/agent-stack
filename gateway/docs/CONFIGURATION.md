...
Create `~/.config/opencode/alloy.json` (or `.opencode/alloy.json` in project root):
...
OPENCODE_ALLOY_DEBUG=1 opencode   # Basic logging
...
Logs are written to `~/.config/opencode/alloy-logs/` (or `log_dir` if set).
...
```bash
OPENCODE_ALLOY_QUIET=1                              # quiet_mode
OPENCODE_ALLOY_DEBUG=1                              # debug (1=basic, 2=verbose)
OPENCODE_ALLOY_LOG_DIR=/path                        # log_dir
OPENCODE_ALLOY_KEEP_THINKING=1                      # keep_thinking
OPENCODE_ALLOY_ACCOUNT_SELECTION_STRATEGY=round-robin
