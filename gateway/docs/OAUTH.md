...
| 6 | `alloy/oauth.ts` → `exchangeAlloy AI()` | Sends `code` + `verifier` + `client_secret` to Alloy AI token endpoint |
...
| `ALLOY_CLIENT_SECRET` | From `AG_CLIENT_SECRET` env or fallback | Client secret for token exchange |
...
### Config File (`~/.config/opencode/alloy.json`)
...
Location: `~/.config/opencode/alloy-accounts.json`
