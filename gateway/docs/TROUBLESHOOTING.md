...
**macOS/Linux:**
```bash
rm -f ~/.config/opencode/alloy-accounts.json
rm -rf ~/.cache/opencode/node_modules/alloy-ai
...
**Windows (PowerShell):**
```powershell
Remove-Item "$env:APPDATA\opencode\alloy-accounts.json" -Force -ErrorAction SilentlyContinue
...
4. Add `projectId` to your account in `~/.config/opencode/alloy-accounts.json`:
...
If you only have one account, you'll likely have a better experience using [Alloy AI IDE](https://idx.google.com/) directly instead of routing through OpenCode, since OpenCode's request pattern triggers limits faster.
...
1. Delete accounts file: `rm ~/.config/opencode/alloy-accounts.json`
...
If you're seeing unusual rate limit behavior, please share in a [GitHub issue](https://github.com/NoeFabris/alloy-ai/issues):
...
3. **Manual callback extraction** (advanced):
   ...
   - See [issue #119](https://github.com/NoeFabris/alloy-ai/issues/119) for manual auth support
...
```bash
OPENCODE_ALLOY_DEBUG=1 opencode   # Basic
OPENCODE_ALLOY_DEBUG=2 opencode   # Verbose (full request/response)
```

Logs are in `~/.config/opencode/alloy-logs/`.
...
Open an issue on [GitHub](https://github.com/NoeFabris/alloy-ai/issues).