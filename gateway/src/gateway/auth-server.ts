...
import { type ProviderAdapter, type UnifiedToken } from "./provider-types";
...
<body>
  ...
  <p>${providerName} hesabi baglandi. Bu pencereyi kapatabilirsiniz.</p>
</body>
</html>`;
    ...
    return new Promise((resolve, reject) => {
      ...
      this.server = http.createServer(async (req, res) => {
        ...
        try {
          ...
          // We use addOrUpdateAccount but it expects StoredToken.
          // UnifiedToken is a superset of StoredToken properties we need.
          this.tokenStore.addOrUpdateAccount(token);
          
          console.log(`[AuthServer] Auth basarili! Email: ${token.email}, Provider: ${token.provider}`);

          res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
          res.end(SUCCESS_HTML(this.adapter.provider));
          clearTimeout(timeout);
