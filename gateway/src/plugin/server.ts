...
import { ALLOY_REDIRECT_URI } from "../constants";
...
const redirectUri = new URL(ALLOY_REDIRECT_URI);
 ...
 * 
 * Priority:
 * 1. OPENCODE_ALLOY_OAUTH_BIND environment variable (user override)
...
function getBindAddress(): string {
  ...
  const envBind = process.env.OPENCODE_ALLOY_OAUTH_BIND;
  if (envBind) {
    return envBind;
  }
...
/**
 * Starts a lightweight HTTP server that listens for the Alloy OAuth redirect
...
<html lang="en">
  ...
  <body>
    <div class="card">
      ...
      <h1>All set!</h1>
      <p>You've successfully authenticated with Alloy. You can now return to Opencode.</p>
