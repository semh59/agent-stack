...
describe("AccountManager", () => {
  ...
  it("removeAccountByEmail keeps active account stable when removing earlier index", () => {
    const stored: AccountStorageV3 = {
      version: 3,
      accounts: [
        { email: "a@example.com", refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
        { email: "b@example.com", refreshToken: "r2", projectId: "p2", addedAt: 1, lastUsed: 0 },
        { email: "c@example.com", refreshToken: "r3", projectId: "p3", addedAt: 1, lastUsed: 0 },
      ],
      activeIndex: 1,
    ...
    expect(manager.removeAccountByEmail("a@example.com")).toBe(true);
    expect(manager.getCurrentOrNextForFamily("claude")?.parts.refreshToken).toBe("r2");

    const snapshot = manager.getAccountsSnapshot();
    expect(snapshot).toHaveLength(2);
    expect(snapshot[0]?.email).toBe("b@example.com");
    expect(snapshot[0]?.index).toBe(0);
    expect(snapshot[1]?.email).toBe("c@example.com");
    expect(snapshot[1]?.index).toBe(1);
  });

  it("removeAccountByEmail resets family pointers when account list becomes empty", () => {
    const stored: AccountStorageV3 = {
      version: 3,
      accounts: [
        { email: "solo@example.com", refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
      ],
      activeIndex: 0,
    ...
    const manager = new AccountManager(undefined, stored);
    expect(manager.removeAccountByEmail("solo@example.com")).toBe(true);
  ...
  it("removeAccountByEmail returns false for unknown email", () => {
    const stored: AccountStorageV3 = {
      version: 3,
      accounts: [
        { email: "known@example.com", refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
      ],
      activeIndex: 0,
    };

    const manager = new AccountManager(undefined, stored);
    expect(manager.removeAccountByEmail("missing@example.com")).toBe(false);
  ...
  describe("header style fallback for Gemini", () => {
    it("tracks rate limits separately for each header style", () => {
      ...
      manager.markRateLimited(account!, 60000, "gemini", "Alloy");

      expect(manager.isRateLimitedForHeaderStyle(account!, "gemini", "Alloy")).toBe(true);
      expect(manager.isRateLimitedForHeaderStyle(account!, "gemini", "gemini-cli")).toBe(false);
    });

    it("getAvailableHeaderStyle returns Alloy first for Gemini", () => {
      const stored: AccountStorageV3 = {
        version: 3,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
      ...
      expect(manager.getAvailableHeaderStyle(account!, "gemini")).toBe("Alloy");
    });

    it("getAvailableHeaderStyle returns gemini-cli when Alloy is rate-limited", () => {
      const stored: AccountStorageV3 = {
        version: 3,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
    ...
    it("getAvailableHeaderStyle always returns Alloy for Claude", () => {
      const stored: AccountStorageV3 = {
        version: 3,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
      ...
      expect(manager.getAvailableHeaderStyle(account!, "claude")).toBe("Alloy");
    });
    ...
    it("getAvailableHeaderStyle returns null for Claude when rate-limited", () => {
      ...
      manager.markRateLimited(account!, 60000, "claude", "Alloy");
    ...
    it("Gemini rate limits expire independently per header style", () => {
      ...
      manager.markRateLimited(account!, 30000, "gemini", "Alloy");
      manager.markRateLimited(account!, 60000, "gemini", "gemini-cli");

      vi.setSystemTime(new Date(35000));

      expect(manager.isRateLimitedForHeaderStyle(account!, "gemini", "Alloy")).toBe(false);
  ...
  describe("touchedForQuota tracking", () => {
    it("marks account as touched with timestamp", () => {
      ...
      manager.markTouchedForQuota(account, "claude:Alloy");

      expect(account.touchedForQuota["claude:Alloy"]).toBe(1000);
    });
    ...
    it("isFreshForQuota returns true for untouched accounts", () => {
      ...
      expect(manager.isFreshForQuota(account, "claude:Alloy")).toBe(true);
    });
    ...
    it("isFreshForQuota returns false for recently touched accounts", () => {
      ...
      expect(manager.isFreshForQuota(account, "claude:Alloy")).toBe(false);
    });
  ...
  describe("Issue #147: headerStyle-aware account selection", () => {
    it("skips account when requested headerStyle is rate-limited even if other style is available", () => {
      ...
      // Mark ONLY Alloy as rate-limited (gemini-cli is still available)
      manager.markRateLimited(firstAccount!, 60000, "gemini", "Alloy");

      // Verify: Alloy is limited, gemini-cli is not
      expect(manager.isRateLimitedForHeaderStyle(firstAccount!, "gemini", "Alloy")).toBe(true);
      expect(manager.isRateLimitedForHeaderStyle(firstAccount!, "gemini", "gemini-cli")).toBe(false);

      // BUG: When we explicitly request Alloy headerStyle, 
      ...
      const nextAccount = manager.getCurrentOrNextForFamily(
        ...
        null, 
        "sticky", 
        "Alloy"  // Explicitly requesting Alloy
      );

      // Verifies headerStyle-aware account selection: should skip account 0
      // because its Alloy quota is limited, even though gemini-cli is available
    ...
    it("returns same account when a different headerStyle is rate-limited", () => {
      ...
      // Mark gemini-cli as rate-limited (Alloy is still available)
      manager.markRateLimited(firstAccount!, 60000, "gemini", "gemini-cli");

      // When requesting Alloy, should return the same account
      // because Alloy quota is still available
      const nextAccount = manager.getCurrentOrNextForFamily(
        "gemini", 
        null, 
        "sticky", 
        "Alloy"  // Requesting Alloy which is NOT limited
      );
  ...
  describe("Rate Limit Reason Classification", () => {
    it("getMinWaitTimeForFamily respects strict header style", () => {
      ...
      manager.markRateLimited(account!, 30000, "gemini", "Alloy", "gemini-3-pro-image");
    ...
    describe("markRateLimitedWithReason", () => {
      it("tracks consecutive failures and applies escalating backoff", () => {
        ...
        const backoff1 = manager.markRateLimitedWithReason(
          account, "gemini", "Alloy", null, "QUOTA_EXHAUSTED"
        );
      ...
      it("uses provided retryAfterMs over calculated backoff", () => {
        ...
        const backoff = manager.markRateLimitedWithReason(
          account, "gemini", "Alloy", null, "QUOTA_EXHAUSTED", 180_000
        );
    ...
    describe("Optimistic Reset", () => {
      it("shouldTryOptimisticReset returns true when min wait time <= 2s", () => {
        ...
        const stored: AccountStorageV3 = {
          version: 3,
          accounts: [
            { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0, rateLimitResetTimes: { "gemini-Alloy": 11_500, "gemini-cli": 11_500 } },
          ],
          activeIndex: 0,
        };
      ...
      it("shouldTryOptimisticReset returns false when min wait time > 2s", () => {
        ...
        const stored: AccountStorageV3 = {
          version: 3,
          accounts: [
            { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0, rateLimitResetTimes: { "gemini-Alloy": 15_000, "gemini-cli": 15_000 } },
          ],
          activeIndex: 0,
        };
      ...
      it("clearAllRateLimitsForFamily clears rate limits and failure counters", () => {
        ...
        const stored: AccountStorageV3 = {
          version: 3,
          accounts: [
            { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0, rateLimitResetTimes: { "gemini-Alloy": 70_000, "gemini-cli": 80_000 } },
            { refreshToken: "r2", projectId: "p2", addedAt: 2, lastUsed: 0, rateLimitResetTimes: { "gemini-Alloy": 90_000 } },
          ],
          activeIndex: 0,
        };
        ...
        expect(accounts[0]!.rateLimitResetTimes["gemini-Alloy"]).toBeUndefined();
        expect(accounts[0]!.rateLimitResetTimes["gemini-cli"]).toBeUndefined();
        expect(accounts[1]!.rateLimitResetTimes["gemini-Alloy"]).toBeUndefined();
  ...
  describe("Failure TTL Expiration", () => {
    it("resets consecutiveFailures when lastFailureTime exceeds TTL", () => {
      ...
      // First failure
      manager.markRateLimitedWithReason(account!, "claude", "Alloy", null, "QUOTA_EXHAUSTED", null, null, 3600_000);
      expect(account!.consecutiveFailures).toBe(1);
  ...
  describe("soft quota threshold", () => {
    it("skips account over soft quota threshold in sticky mode", () => {
      ...
      const account = manager.getCurrentOrNextForFamily("claude", null, "sticky", "Alloy", false, 90);
    ...
    it("threshold of 100 disables soft quota protection", () => {
      ...
      const account = manager.getCurrentOrNextForFamily("claude", null, "sticky", "Alloy", false, 100);
      expect(account?.parts.refreshToken).toBe("r1");
    });
    ...
    it("skips account over threshold in round-robin mode", () => {
      ...
      const account = manager.getCurrentOrNextForFamily("claude", null, "round-robin", "Alloy", false, 90);
      expect(account?.parts.refreshToken).toBe("r2");
    });
