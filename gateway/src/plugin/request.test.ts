...
describe("request.ts", () => {
  ...
  describe("prepareAlloyRequest", () => {
    ...
    it("handles wrapped Alloy body format", () => {
      const wrappedBody = {
        project: "my-project",
    ...
    describe("Issue #103: model name transformation during quota fallback", () => {
      it("transforms gemini-3-flash-preview to gemini-3-flash for Alloy headerStyle", () => {
        const result = prepareAlloyRequest(
      ...
      it("transforms gemini-3-pro-preview to gemini-3-pro-low for Alloy headerStyle", () => {
        const result = prepareAlloyRequest(
    ...
    // Bug #4 Fix Tests: Tool ID Assignment with Defensive Shift
    describe("Tool ID Assignment - Defensive Shift (Bug #4)", () => {
      it("defensive shift guards against undefined ID assignment", () => {
        // Simulate the exact defensive pattern implemented for Bug #4
        const pendingCallIdsByName = new Map<string, string[]>();
        pendingCallIdsByName.set("my_func", ["id_1", "id_2"]);

        const resp: any = { name: "my_func" };

        // Apply the defensive shift pattern
        ...
        // Verify correct ID was assigned
        expect(resp.id).toBe("id_1");
        expect(queue?.length).toBe(1); // One ID consumed
      });

      it("does not assign ID if queue becomes empty between check and shift", () => {
        // Defensive shift prevents undefined assignment
        const queue: string[] = [];

        const resp: any = { name: "unknown" };

        // Explicitly test the defensive pattern
        if (queue && queue.length > 0) {
          const id = queue.shift();
          if (id !== undefined) {
            resp.id = id;
        ...
        // Should not have assigned any ID
        expect(resp.id).toBeUndefined();
      });

      it("handles multiple tool responses with FIFO ordering", () => {
        const pendingCallIdsByName = new Map<string, string[]>();
        pendingCallIdsByName.set("func_a", ["a1", "a2"]);
        pendingCallIdsByName.set("func_b", ["b1"]);

        // Process response for func_a
        const resp1: any = { name: "func_a" };
        let queue = pendingCallIdsByName.get(resp1.name);
        if (queue && queue.length > 0) {
          const id = queue.shift();
          if (id !== undefined) {
            resp1.id = id;
        ...
        // Process response for func_b
        const resp2: any = { name: "func_b" };
        queue = pendingCallIdsByName.get(resp2.name);
        if (queue && queue.length > 0) {
          const id = queue.shift();
          if (id !== undefined) {
            resp2.id = id;
        ...
        // Verify FIFO order
        expect(resp1.id).toBe("a1");
        expect(resp2.id).toBe("b1");
        expect(pendingCallIdsByName.get("func_a")?.length).toBe(1); // a2 remains
        expect(pendingCallIdsByName.get("func_b")?.length).toBe(0); // b1 consumed
      });
    });
  });
});
