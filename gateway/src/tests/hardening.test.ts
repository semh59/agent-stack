...
describe('Alloy Hardening Suite', () => {
  ...
  describe('Response Validation (Zod)', () => {
    it('should pass for valid Alloy responses', () => {
      const validResponse = {
        type: 'message',
        message: { content: 'Hello' }
      };
      const result = AlloyResponseSchema.safeParse(validResponse);
    ...
    it('should pass even with extra fields (passthrough)', () => {
      ...
      const result = AlloyResponseSchema.safeParse(respWithExtras);
      expect(result.success).toBe(true);
    });
    ...
    it('should fail if type is not a string', () => {
      ...
      const result = AlloyResponseSchema.safeParse(invalidResp);
