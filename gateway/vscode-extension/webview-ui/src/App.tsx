...
function TabBar() {
  ...
  return (
    <div className="flex shrink-0 border-b border-[var(--alloy-border-subtle)] bg-[var(--alloy-bg-secondary)]">
      <button
        ...
        className={cn(
          ...
          activeView === "chat"
            ...
            : "border-transparent text-[var(--alloy-text-muted)] hover:text-[var(--alloy-text-secondary)]"
        )}
      >
      ...
      >
        ...
        Accounts
        {hasIssue ? (
          <span className="w-1.5 h-1.5 rounded-full bg-[var(--alloy-error)] absolute top-1.5 right-1" />
        ) : activeCount > 0 ? (
          <span className="ml-0.5 px-1 py-0.5 rounded-full bg-[var(--alloy-accent-subtle)] text-[var(--alloy-accent)] text-[9px] font-bold leading-none">
            {activeCount}
          </span>
        ) : (
          <span className="w-1.5 h-1.5 rounded-full bg-[var(--alloy-text-muted)] opacity-40" />
        )}
      </button>
    </div>
  );
    ...
    (msg: IncomingMessage) => {
      switch (msg.type) {
        ...
        case "system":
          ...
          if (chatStore.isAddingAccount) chatStore.setAddingAccount(false);
          break;
    ...
    <div className="flex flex-col h-full bg-[var(--alloy-bg-primary)]">
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {activeView === "chat" ? (
          ...
          <AccountPanel postMessage={postMessage} />
        )}
      </div>
    </div>
  );
