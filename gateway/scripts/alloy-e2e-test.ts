...
async function main() {
  ...
  const benchmarkDir = path.join(projectRoot, 'alloy-benchmark-api');
  ...
  const benchmarkData = {
    default: "I am a alloy agent. I will proceed with the task as instructed.",
...
`
  ...
  try {
    ...
    if (result.status === 'completed') {
      console.log('✅ PASS: Alloy E2E Benchmark Successful.');
    } else {
      console.log('❌ FAIL: Alloy E2E Benchmark failed.');
      process.exit(1);
    }
  } catch (err) {
