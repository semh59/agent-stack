...
async function restore() {
  ...
  for (const skill of MISSING_SKILLS) {
    ...
    if (!fs.existsSync(file) || fs.readFileSync(file, 'utf-8').includes('[Alloy Restoration in Progress]')) {
      console.log(`[SkillRestorer] Generating content for: ${skill}...`);
      
      const systemPrompt = `You are a Senior Expert in ${skill}. 
Your task is to write a comprehensive SKILL.md for the Alloy AI v4 project.
...
Keep it concise but highly technical.`;
      ...
      // but in a real alloy run we would use fetch here.
