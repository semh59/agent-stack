...
Enable Opencode to authenticate against **Alloy AI** (Google's IDE) via OAuth so you can use Alloy AI rate limits and access models like `gemini-3-pro` and `claude-opus-4-5-thinking` with your Google credentials.

> Migration note: older docs may reference `opencode-alloy-auth`; the active package name is `alloy-ai`.
...
```
Install the alloy-ai plugin and add the Alloy AI model definitions to ~/.config/opencode/opencode.json by following: https://raw.githubusercontent.com/NoeFabris/alloy-ai/dev/README.md
```
...
4. **Use it:**
   ...
   ```bash
   opencode run "Hello" --model=google/alloy-claude-sonnet-4-5-thinking --variant=max
   ```
...
> **Quick Reset**: Most issues can be resolved by deleting `~/.config/opencode/alloy-accounts.json` and running `opencode auth login` again.
...
| Accounts | `~/.config/opencode/alloy-accounts.json` |
| Plugin config | `~/.config/opencode/alloy.json` |
| Debug logs | `~/.config/opencode/alloy-logs/` |
...
1. Delete the accounts file:
   ```bash
   rm ~/.config/opencode/alloy-accounts.json
   ```
2. Re-authenticate:
   ```bash
   opencode auth login
...
2. Copy `~/.config/opencode/alloy-accounts.json`
...
Create `~/.config/opencode/alloy.json` for optional settings:
...
OPENCODE_ALLOY_DEBUG=1 opencode         # Enable debug logging
OPENCODE_ALLOY_DEBUG=2 opencode         # Verbose logging
```
