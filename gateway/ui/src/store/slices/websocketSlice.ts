...
declare global {
  interface Window {
    __alloy_ws?: WebSocket;
  }
}

const AUTONOMY_CLIENT_ID_KEY = "alloy_autonomy_client_id";
const AUTONOMY_GENERATION_KEY = "alloy_autonomy_generation";
...
> = (set, get) => ({
  ...
  initializeWebSocket: () => {
    ...
    const connectWS = () => {
      ...
      if (window.__alloy_ws) {
        window.__alloy_ws.close();
      }

      const ws = new WebSocket(`${GATEWAY_WS_LOGS_URL}?token=${encodeURIComponent(token)}`);
      window.__alloy_ws = ws;

      ws.onmessage = (event) => {
        try {
           if (window.__alloy_ws !== ws) return;
      ...
      ws.onclose = () => {
         if (window.__alloy_ws === ws) {
            window.__alloy_ws = undefined;
