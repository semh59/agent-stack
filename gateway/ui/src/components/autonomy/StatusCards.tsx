  ...
  return (
    <section className={cardClasses()} aria-label="Faz gostergesi karti">
      <div className="mb-2 flex items-center justify-between gap-3">
        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--color-alloy-text-sec)]">
          Faz Gostergesi
        </span>
        <span className="rounded-full border border-[var(--color-alloy-accent)]/30 bg-[var(--color-alloy-accent)]/10 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-[var(--color-alloy-accent)]">
          {label}
        </span>
      </div>
      <p className="text-sm font-semibold text-white">{label}</p>
      <p className="mt-2 text-xs leading-relaxed text-[var(--color-alloy-text-sec)]">{note}</p>
    </section>
  );
}
...
export function GearStatusCard({ session }: { session: AutonomySessionSummary | null }) {
  ...
  return (
    <section className={cardClasses()} aria-label="Disli durumu karti">
      ...
      <p className="mt-2 text-xs leading-relaxed text-[var(--color-alloy-text-sec)]">{model}</p>
    </section>
  );
}
