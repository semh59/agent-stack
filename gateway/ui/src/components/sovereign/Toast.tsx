...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: (id: string) => void }) {
  ...
  return (
    <div
      role="status"
      className={clsx(
        "pointer-events-auto w-96 max-w-[90vw] rounded-xl border bg-[var(--color-alloy-surface)] p-4 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.9)]",
        toast.tone === "success" && "border-emerald-500/40",
        toast.tone === "warning" && "border-amber-500/40",
        toast.tone === "error" && "border-red-500/40",
        toast.tone === "info" && "border-[var(--color-alloy-border)]",
      )}
    >
      <div className="flex items-start gap-3">
        <Icon
          ...
          className={clsx(
            ...
            toast.tone === "info" && "text-[var(--color-alloy-accent)]",
          )}
        />
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-white">{toast.title}</p>
          {toast.description ? (
            <p className="mt-1 text-xs text-[var(--color-alloy-text-sec)]">{toast.description}</p>
          ) : null}
        </div>
        <button
          type="button"
          onClick={() => onDismiss(toast.id)}
          className="shrink-0 rounded p-1 text-[var(--color-alloy-text-sec)] hover:text-white"
          aria-label="Dismiss"
        >
          <X size={14} />
        </button>
