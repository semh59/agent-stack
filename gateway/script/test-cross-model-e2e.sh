...
opencode run -m google/alloy-gemini-3-pro-low \
...
if [ -z "$SID" ]; then
  ...
  opencode run -s "$SID" -m google/alloy-claude-opus-4-5-thinking-low \
...
if [ -z "$SID" ]; then
  ...
  opencode run -s "$SID" -m google/alloy-gemini-3-pro-low \
...
if grep -qi "api.*key\|unauthorized\|authentication" /tmp/e2e-t5-s1.log; then
  ...
  if [ -z "$SID" ]; then
    ...
    opencode run -s "$SID" -m google/alloy-claude-sonnet-4-5-thinking-low \
...
if [ -z "$SID" ]; then
  ...
  opencode run -s "$SID" -m google/alloy-gemini-3-flash \
...
opencode run -m google/alloy-claude-sonnet-4-5-thinking-low \
...
opencode run -m google/alloy-gemini-3-flash \
