...
describe("AntigravityAPI", () => {
  ...
  it("simulates a 10-second latency window before dispatching the request", async () => {
    ...
    const responsePromise = api.fetch(MODEL_URL, {
      ...
      headers: {
        ...
        "x-antigravity-mock-latency-ms": "10000",
      },
    ...
    expect(
      new Headers(nativeFetch.mock.calls[0]?.[1]?.headers).has("x-antigravity-mock-latency-ms"),
    ).toBe(false);
  ...
  it("returns a graceful 401 response when token refresh fails mid-mission", async () => {
    ...
    expect(response.headers.get("X-Antigravity-Graceful-Error")).toBe("true");
  ...
  it("rotates accounts immediately on 429 before sleeping on the same account", async () => {
    ...
    const api = new AntigravityAPI(manager, {} as any, "antigravity", async () => createOAuthAuth(), nativeFetch);
    const response = await api.fetch(MODEL_URL, {
      method: "POST",
      headers: { Authorization: "Bearer token-1" },
    });
