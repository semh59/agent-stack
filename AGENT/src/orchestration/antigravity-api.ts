...
const MOCK_LATENCY_HEADER = "x-antigravity-mock-latency-ms";
...
export class AntigravityAPI {
  ...
  public async fetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    const headerStyle = fetchInputString.includes("gemini-cli") ? "gemini-cli" : "antigravity";
    ...
    while (attempts < MAX_RETRY_ATTEMPTS) {
      ...
      if (response.status === 429) {
        ...
        if (this.accountManager?.getCurrentOrNextForFamily) {
          ...
          if (next && nextAccessToken && next.index !== current?.index) {
            log.warn(`[Antigravity] 429 hit. Rotating from ${current?.email || 'unknown'} to ${next.email || next.index}`);
      ...
      if (response.status === 401 && !refreshAttempted) {
        ...
        log.info(
          `[Antigravity] Received 401${locallyExpired ? " for expired access token" : ""}, attempting refresh...`,
        );
        try {
...
function createGracefulErrorResponse(message: string, status: number): Response {
  return new Response(JSON.stringify({ error: { message } }), {
    ...
    headers: {
      ...
      "X-Antigravity-Graceful-Error": "true",
    },
  });
}
