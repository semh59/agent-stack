...
describe("D3 Bug #6: storage write survives crashes and sweeps orphans", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "sovereign-d3-"));
  ...
  it("removes orphaned .tmp files older than the grace window on first save", async () => {
    const storageFile = join(tempDir, "Sovereign-accounts.json");
    writeFileSync(storageFile, "{}");

    const staleTmp = join(tempDir, "Sovereign-accounts.json.aaaaaa.tmp");
    const freshTmp = join(tempDir, "Sovereign-accounts.json.bbbbbb.tmp");
    ...
    expect(remaining.includes("Sovereign-accounts.json.aaaaaa.tmp")).toBe(false);
