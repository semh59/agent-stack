...
interface SovereignTokenExchangeSuccess {
  type: "success";
  refresh: string;
  access: string;
  expires: number;
...
interface SovereignTokenExchangeFailure {
  type: "failed";
  error: string;
}

export type SovereignTokenExchangeResult =
  | SovereignTokenExchangeSuccess
  | SovereignTokenExchangeFailure;

interface SovereignTokenResponse {
  access_token: string;
  expires_in: number;
  refresh_token: string;
}

interface SovereignUserInfo {
  email?: string;
}
 ...
 */
function encodeState(payload: SovereignAuthState): string {
 ...
 */
function decodeState(state: string): SovereignAuthState {
...
/**
 * Build the Sovereign OAuth authorization URL using server-side PKCE state.
 * The PKCE verifier is stored server-side and NOT exposed in the URL.
 */
export async function authorizeGoogleGemini(projectId = ""): Promise<SovereignAuthorization> {
  ...
  url.searchParams.set("client_id", SOVEREIGN_CLIENT_ID);
  url.searchParams.set("response_type", "code");
  url.searchParams.set("redirect_uri", SOVEREIGN_REDIRECT_URI);
  url.searchParams.set("scope", SOVEREIGN_SCOPES.join(" "));
...
async function fetchProjectID(accessToken: string): Promise<string> {
  ...
  if (errors.length) {
    log.warn("Failed to resolve Sovereign project via loadCodeAssist", { errors: errors.join("; ") });
  }
  return "";
}

/**
 * Exchange an authorization code for Sovereign CLI access and refresh tokens.
 */
  ...
  try {
    ...
    const tokenResponse = await fetch("https://oauth2.googleapis.com/token", {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: SOVEREIGN_REDIRECT_URI,
    ...
    const tokenPayload = (await tokenResponse.json()) as SovereignTokenResponse;
    ...
    const userInfo = userInfoResponse.ok
      ? ((await userInfoResponse.json()) as SovereignUserInfo)
      : {};
