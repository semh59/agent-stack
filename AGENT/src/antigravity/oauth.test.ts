...
import { pkceStateManager, authorizeAntigravity, exchangeAntigravity } from './oauth';

vi.mock('../constants', () => ({
  ANTIGRAVITY_CLIENT_ID: 'test-client-id',
  ANTIGRAVITY_CLIENT_SECRET: 'test-client-secret',
  ANTIGRAVITY_REDIRECT_URI: 'http://localhost:3000/callback',
  ANTIGRAVITY_SCOPES: ['scope1', 'scope2'],
  ANTIGRAVITY_ENDPOINT_FALLBACKS: [],
  ANTIGRAVITY_LOAD_ENDPOINTS: ['https://api.example.com'],
  ANTIGRAVITY_HEADERS: { 'Client-Metadata': 'test' },
...
describe('OAuth Authorization Flow', () => {
  ...
  it('generates authorization URL with server-side PKCE', async () => {
    const auth = await authorizeAntigravity('test-project');
...
describe('OAuth Token Exchange', () => {
  ...
  it('exchanges code for tokens using server-side verifier', async () => {
    ...
    const result = await exchangeAntigravity('test-code', state);
  ...
  it('rejects invalid state', async () => {
    ...
    const result = await exchangeAntigravity('test-code', 'invalid-state');
  ...
  it('prevents state replay attacks during token exchange', async () => {
    ...
    const result1 = await exchangeAntigravity('code-1', state);
    expect(result1.type).toBe('success');

    // Replay attempt with same state - should fail
    const result2 = await exchangeAntigravity('code-2', state);
  ...
  it('handles token endpoint errors gracefully', async () => {
    ...
    const result = await exchangeAntigravity('invalid-code', state);
