...
import { isMissionModel, toMissionModel } from "../models/mission.model";
...
export class MissionServiceError extends Error {
  constructor(
    public readonly code: MissionServiceErrorCode,
    message?: string,
...
function missionNotFound(id: string): MissionServiceError {
  return new MissionServiceError("MISSION_NOT_FOUND", `Mission ${id} was not found`);
}

function invalidTransition(id: string, message: string): MissionServiceError {
  return new MissionServiceError("INVALID_STATE_TRANSITION", `Mission ${id}: ${message}`);
}

function runtimeFailure(id: string, message: string, cause?: unknown): MissionServiceError {
  return new MissionServiceError("MISSION_RUNTIME_ERROR", `Mission ${id}: ${message}`, {
    cause: cause instanceof Error ? cause : undefined,
  });
}

function planNotAvailable(id: string): MissionServiceError {
  return new MissionServiceError("PLAN_NOT_AVAILABLE", `Mission ${id} has no plan output yet`);
}
...
export class MissionService {
  ...
  public async create(input: CreateMissionInput): Promise<MissionModel> {
    return this.withUnitOfWork(async () => {
      ...
      const mission = toMissionModel(session);
      ...
      try {
        ...
        void this.cancelOrphanRuntimeMission(session.id);
        throw error;
      }
    });
  }
  ...
  private async loadMission(id: string): Promise<MissionModel | null> {
    ...
    const runtimeMission = runtimeSession ? toMissionModel(runtimeSession) : null;
  ...
  private async syncMissionFromRuntimeBestEffort(id: string): Promise<void> {
    try {
      ...
      const runtimeMission = toMissionModel(runtimeSession);
  ...
  private async withUnitOfWork<T>(operation: () => Promise<T>): Promise<T> {
    try {
      ...
      try {
        ...
        // Preserve the original failure; rollback semantics are thin in Phase 3.2.
      }
      throw error;
    }
  }
