...
Plugin modülü, OpenCode'dan gelen istekleri Sovereign AI API formatına dönüştürür, yanıtları geri dönüştürür. SSE streaming, thinking bloğu yönetimi, schema temizleme ve oturum kurtarma içerir.
...
├── sovereign/oauth.ts         ← OAuth akışı
└── plugin/
...
| 2A-3 | Header stili belirleme (sovereign/gemini-cli) | ✅ | `fetch-helpers.ts:82-92` → `getHeaderStyleFromUrl()` |
| 2A-4 | Aktif hesap al/Token yenile | ✅ | `plugin.ts` iç `fetch` döngüsünde hesap seçimi + `token.ts` ile yenileme |
| 2A-5 | Endpoint fallback: daily → autopush → prod | ✅ | `constants.ts:41-45` → `SOVEREIGN_ENDPOINT_FALLBACKS` + circuit breaker |
...
| `sovereign-first-fallback.test.ts` | Endpoint fallback entegrasyon |
...
| 2A | Endpoint fallback | `SOVEREIGN_ENDPOINT_FALLBACKS` | constants.ts | 41 | ✅ fallback.test.ts |
