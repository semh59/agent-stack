...
import { Badge } from "../../../../components/sovereign/primitives";
import { FormattedMessage } from "./message-format";

export function MessageList() {
  const { conversations, activeConversationId } = useSovereignStore();
          ...
          >
            <div
              className={clsx(
                ...
                msg.role === "user"
                  ? "border-[var(--color-loji-border)] bg-[var(--color-loji-surface)] text-[var(--color-loji-text)]"
            ...
            <div
              className={clsx(
                ...
                msg.role === "user"
                  ? "border-[var(--color-loji-border)] bg-[var(--color-loji-surface)]"
                  : "border-[var(--color-loji-accent)]/20 bg-[var(--color-loji-accent)]/5",
              )}
            >
              <div className="mb-1 flex items-center gap-2 text-[10px] uppercase tracking-widest text-[var(--color-loji-text-sec)]">
              ...
              {(msg.tokens?.input || msg.tokens?.output) && !msg.pending ? (
                <div className="mt-2 flex items-center gap-3 text-[10px] text-[var(--color-loji-text-sec)]">
                  <Sparkles size={10} />
                  <span>
  ...
  return (
    <div className="flex min-h-0 flex-1 items-center justify-center p-10">
      <div className="max-w-md space-y-2 text-center">
        <Bot size={40} className="mx-auto text-[var(--color-loji-accent)]" />
        <h2 className="font-display text-xl tracking-wide text-white">
          Sovereign is ready.
        </h2>
        <p className="text-sm text-[var(--color-loji-text-sec)]">
  ...
  return (
    <div className="flex min-h-0 flex-1 items-center justify-center p-10">
      <div className="max-w-md space-y-3 text-center">
        <Sparkles size={32} className="mx-auto text-[var(--color-loji-accent)]" />
        ...
        <p className="text-sm text-[var(--color-loji-text-sec)]">
          ...
          <br />
          <span className="mt-2 inline-block italic text-[var(--color-loji-accent)]">
