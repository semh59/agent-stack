...
export function Composer() {
  const { sendingMessage, sendError, sendMessage } = useSovereignStore();
  ...
  return (
    <div className="border-t border-[var(--color-loji-border)] bg-[var(--color-loji-surface)]/60 backdrop-blur-sm">
      <div className="mx-auto max-w-3xl px-6 py-4">
        ...
        <div
          className={clsx(
            "relative rounded-2xl border bg-[var(--color-loji-bg)] shadow-[0_10px_40px_-20px_rgba(0,0,0,0.8)] transition-colors",
            sendingMessage
              ? "border-[var(--color-loji-accent)]/40"
              : "border-[var(--color-loji-border)] focus-within:border-[var(--color-loji-accent)]/40",
          )}
        >
          <textarea
            ref={textareaRef}
            ...
            onKeyDown={onKeyDown}
            placeholder="Ask Sovereign anything — it will route through the optimization pipeline first."
            disabled={sendingMessage}
            rows={MIN_ROWS}
            className="block w-full resize-none bg-transparent px-4 pt-3 pb-2 font-body text-sm text-white placeholder:text-[var(--color-loji-text-sec)] focus:outline-none"
          />

          <div className="flex items-center justify-between gap-2 border-t border-[var(--color-loji-border)]/60 px-3 py-2">
            <div className="flex items-center gap-2">
              <ModelPicker value={model} onChange={setModel} />
              <span className="hidden text-[11px] text-[var(--color-loji-text-sec)] md:inline">
