...
function ProviderCard({ def }: { def: ProviderDef }) {
  ...
  const { updateSettingsPath, probeProvider, providerProbes } = useSovereignStore();
  ...
  return (
    <Card density="compact" tone={enabled ? "accent" : "neutral"}>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-[var(--color-loji-text-sec)]">{def.blurb}</p>
          </div>
        </div>
        <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
          ...
          <ChevronDown
            size={18}
            className={clsx(
              "text-[var(--color-loji-text-sec)] transition-transform",
              expanded && "rotate-180",
            )}
          />
        </div>
      ...
      {expanded ? (
        <div className="mt-5 border-t border-[var(--color-loji-border)] pt-5">
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            {def.helpUrl ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-xs text-[var(--color-loji-accent)] hover:underline"
              >
                Get API key ↗
              </a>
            ) : null}
...
function ProbeBadge({ probe }: { probe: ReturnType<typeof useSovereignStore.getState>["providerProbes"][string] | undefined }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-[var(--color-loji-text-sec)]">· {probe.latency_ms}ms</span>
      </div>
      {probe.detail ? <p className="mt-1">{probe.detail}</p> : null}
      {typeof probe.models_seen === "number" ? (
        <p className="mt-1 text-[var(--color-loji-text-sec)]">
