...
export function PlanApprovalView() {
  ...
  useEffect(() => {
    ...
    if (!sessionId) {
      setScreenError("Plan ekranı için session id zorunlu.");
      setLoading(false);
      return;
    }
  ...
  if (loading) {
    return (
      <div className="flex min-h-[320px] items-center justify-center rounded-2xl border border-white/10 bg-black/20">
        <div className="flex items-center gap-3 text-sm text-[var(--color-loji-text-sec)]">
  ...
  if (!session) {
    return (
      <div className="rounded-2xl border border-white/10 bg-black/20 p-6 text-sm text-[var(--color-loji-text-sec)]">
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-white/10 bg-black/20 p-6 shadow-2xl">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-[10px] font-black uppercase tracking-[0.35em] text-[var(--color-loji-text-sec)]">
              Plan Onay Ekrani
            </div>
            <h2 className="mt-2 text-2xl font-bold text-white">{session.objective || "Mission objective bekleniyor"}</h2>
            <p className="mt-2 text-sm text-[var(--color-loji-text-sec)]">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.5fr_1fr]">
        <div className="space-y-6">
          ...
          <section className="rounded-3xl border border-white/10 bg-black/20 p-6">
            <div className="mb-4 text-[10px] font-black uppercase tracking-[0.35em] text-[var(--color-loji-text-sec)]">
              Plan Summary
            </div>
            {!hasPlan ? (
              <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-[var(--color-loji-text-sec)]">
              ...
              <div className="space-y-4">
                {structuredPlan.map((section) => (
                  <article key={section.title} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                    ...
                    <pre className="mt-3 whitespace-pre-wrap font-mono text-xs leading-relaxed text-[var(--color-loji-text-sec)]">
                      {section.body}
                    </pre>
                  </article>
                ))}
              ...
              <pre className="rounded-2xl border border-white/10 bg-black/20 p-4 whitespace-pre-wrap font-mono text-xs leading-relaxed text-[var(--color-loji-text-sec)]">
                {artifacts?.plan}
              </pre>
            )}
          </section>
        ...
        <div className="space-y-6">
          <section className="rounded-3xl border border-white/10 bg-black/20 p-6">
            ...
            <div className="mt-4 space-y-4 text-sm">
              <div>
                <div className="text-[10px] font-black uppercase tracking-widest text-[var(--color-loji-text-sec)]">Budget</div>
                <p className="mt-2 text-white">{formatBudgetSummary(budget)}</p>
              </div>
              <div>
                <div className="text-[10px] font-black uppercase tracking-widest text-[var(--color-loji-text-sec)]">Gate</div>
                <p className="mt-2 text-white">
                  {gate ? (gate.passed ? "Passed" : `Blocked (${gate.blockingIssues.length})`) : "Gate sonucu bekleniyor."}
                </p>
              </div>
              <div>
                <div className="text-[10px] font-black uppercase tracking-widest text-[var(--color-loji-text-sec)]">Touched Areas</div>
                <ul className="mt-2 space-y-2 text-[var(--color-loji-text-sec)]">
                  {(touchedFiles.length > 0 ? touchedFiles : ["Henüz touched file yok."]).map((filePath) => (
          ...
          <section className="rounded-3xl border border-white/10 bg-black/20 p-6">
            ...
            <dl className="mt-4 space-y-3 text-sm">
              <div>
                <dt className="text-[10px] font-black uppercase tracking-widest text-[var(--color-loji-text-sec)]">Updated</dt>
                <dd className="mt-1 text-white">{session.updatedAt}</dd>
              </div>
              <div>
                <dt className="text-[10px] font-black uppercase tracking-widest text-[var(--color-loji-text-sec)]">Review Updated</dt>
                <dd className="mt-1 text-white">{session.reviewUpdatedAt ?? "N/A"}</dd>
              </div>
              <div>
                <dt className="text-[10px] font-black uppercase tracking-widest text-[var(--color-loji-text-sec)]">Branch</dt>
