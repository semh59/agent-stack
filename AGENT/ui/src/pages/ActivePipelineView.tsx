...
export function ActivePipelineView() {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--color-loji-bg)]">
      <div className="h-16 border-b border-[var(--color-loji-border)] flex items-center justify-between px-6 bg-[var(--color-loji-surface)]">
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 text-xs bg-[var(--color-loji-bg)] border border-[var(--color-loji-border)] px-3 py-1.5 rounded-md"
          >
            {isPaused || autonomySession?.state === "paused" ? (
              <Play size={14} className="text-[var(--color-loji-success)]" />
            ) : (
              <Pause size={14} className="text-[var(--color-loji-warning)]" />
            )}
            {isPaused || autonomySession?.state === "paused" ? "Devam" : "Duraklat"}
          </button>
          <button className="flex items-center gap-2 text-xs bg-[var(--color-loji-bg)] border border-[var(--color-loji-border)] px-3 py-1.5 rounded-md">
            <FastForward size={14} /> Atla
          </button>
          <button
            onClick={handleStop}
            className="flex items-center gap-2 text-xs bg-[var(--color-loji-bg)] border border-red-900/50 text-red-400 px-3 py-1.5 rounded-md"
          >
            <XCircle size={14} /> Iptal
          </button>
          <span className="text-xs text-[var(--color-loji-text-sec)]">{activeAccount || "Hesap yok"}</span>
        </div>
      </div>

      <div className="flex-1 min-h-0 grid grid-cols-[2fr_1fr]">
        <div className="min-h-0 bg-[#0a0a0c] flex flex-col">
          <div className="p-3 border-b border-[var(--color-loji-border)] bg-[var(--color-loji-surface)] text-xs text-[var(--color-loji-text-sec)] flex items-center gap-2">
          ...
          <div className="flex-1 p-4 overflow-y-auto font-body text-xs space-y-1.5">
            {autonomyTimeline.map((entry) => (
              <div key={entry.id} className="flex gap-3">
                <span className="text-[var(--color-loji-text-sec)] shrink-0 opacity-70">[{new Date(entry.timestamp).toLocaleTimeString("en-US", { hour12: false })}]</span>
            ...
            {logs.map((log) => (
              <div key={log.id} className="flex gap-3">
                <span className="text-[var(--color-loji-text-sec)] shrink-0 opacity-70">[{log.time}]</span>
                <span className="font-medium shrink-0 w-24 truncate text-[var(--color-loji-accent)]">[{log.source}]</span>
        ...
        <div className="border-l border-[var(--color-loji-border)] bg-[var(--color-loji-surface)] p-4 space-y-4">
          <PhaseStatusCard session={autonomySession} />
          <GearStatusCard session={autonomySession} />

          <div>
            <span className="text-xs text-[var(--color-loji-text-sec)] uppercase">Gate Status</span>
            <p
              className={clsx(
          ...
          <div>
            <span className="text-xs text-[var(--color-loji-text-sec)] uppercase">Impacted Scopes</span>
            <p className="mt-2 text-sm text-white">{gateStatus?.impactedScopes.join(", ") || "-"}</p>
          </div>

          {gateStatus?.audit ? (
            <div>
              <span className="text-xs text-[var(--color-loji-text-sec)] uppercase">Audit</span>
          ...
          {budgetStatus ? (
            <div>
              <span className="text-xs text-[var(--color-loji-text-sec)] uppercase">Budget</span>
              <p className="mt-2 text-sm text-white">
