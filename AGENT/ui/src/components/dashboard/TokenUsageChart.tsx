    ...
    <div className="glass-panel-elite rounded-2xl p-6 space-y-4 animate-float">
      <div className="flex items-center justify-between">
        <h3 className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] flex items-center gap-2">
          <Activity size={14} className="text-[var(--color-loji-accent)]" /> TOKEN VELOCITY
        </h3>
        <div className="text-[10px] text-white/60 font-mono">
          <span className="text-[var(--color-loji-accent)] font-black text-xs">{(analytics?.tokenVelocity || 0).toLocaleString()}</span> T/SEC
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div className="p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/[0.08] transition-all group">
          <div className="text-[8px] text-white/30 font-black uppercase tracking-widest mb-1 group-hover:text-[var(--color-loji-accent)] transition-colors">Current TPM</div>
