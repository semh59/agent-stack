...
const DEFAULT_CONFIG: BridgeManagerConfig = {
  port: parseInt(process.env.AI_STACK_BRIDGE_PORT ?? "9100", 10),
...
export class BridgeManager implements vscode.Disposable {
  ...
  async start(): Promise<void> {
    ...
    const env = {
      ...
      AI_STACK_BRIDGE_PORT: String(this.config.port),
      AI_STACK_BRIDGE_SECRET: this.bridgeSecret,
    };
  ...
  private async detectPython(): Promise<string> {
    ...
    const venvPaths = [
      path.join(this.workspaceRoot, "ai-stack-mcp", ".venv", "Scripts", "python.exe"),
      path.join(this.workspaceRoot, "ai-stack-mcp", ".venv", "bin", "python"),
  ...
  private resolveBridgePath(): string | null {
    ...
    const searchPaths = [
      path.join(this.workspaceRoot, "ai-stack-mcp", "bridge.py"),
      path.join(this.workspaceRoot, "..", "ai-stack-mcp", "bridge.py"),
    ];
