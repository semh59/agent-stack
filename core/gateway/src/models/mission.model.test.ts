...
describe("mission.model", () => {
  it.each([
    ...
    const mission = MissionFactory.fromSession(createSession(overrides));
  ...
  it("derives plan_review from reviewStatus and keeps review metadata", () => {
    const mission = MissionFactory.fromSession(
      createSession({
        state: "plan",
        reviewStatus: "plan_pending",
  ...
  it("parses structured plan content into MissionPlan", () => {
    const mission = MissionFactory.fromSession(createSession());
  ...
  it("returns null for empty plan payloads", () => {
    expect(MissionFactory.parsePlan("   ")).toBeNull();
  });
});
