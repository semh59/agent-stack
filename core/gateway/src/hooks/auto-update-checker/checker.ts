...
import type { NpmDistTags, AlloyConfig, PackageJson, UpdateCheckResult } from "./types";
import {
  PACKAGE_NAME,
  NPM_REGISTRY_URL,
  NPM_FETCH_TIMEOUT,
...
function getConfigPaths(directory: string): string[] {
  return [
    path.join(directory, ".Alloy", "Alloy.json"),
    path.join(directory, ".Alloy", "Alloy.jsonc"),
    path.join(directory, ".Alloy.json"),
    USER_Alloy_CONFIG,
    USER_Alloy_CONFIG_JSONC,
  ];
}
...
export function getLocalDevPath(directory: string): string | null {
  for (const configPath of getConfigPaths(directory)) {
    try {
      ...
      const config = JSON.parse(stripJsonComments(content)) as AlloyConfig;
