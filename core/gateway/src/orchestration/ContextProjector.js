...
exports.ContextProjector = void 0;
...
class ContextProjector {
     ...
     */
    static async projectRecentActivity(logPath, maxEvents = 20) {
        try {
            const content = await fs.readFile(logPath, 'utf-8');
            const lines = content.split('\n').filter(l => l.trim().length > 0);
            const recentLines = lines.slice(-maxEvents);
            const events = recentLines.map(l => JSON.parse(l));
            if (events.length === 0)
...
exports.ContextProjector = ContextProjector;
//# sourceMappingURL=ContextProjector.js.map