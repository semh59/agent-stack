...
/**
 * PipelineTools: Exposes Sequential Pipeline capabilities as Alloy plugin tools.
 */
export class PipelineTools {
  private pipeline: SequentialPipeline;
  private terminal: TerminalExecutor;
  private projectRoot: string;
  private AlloyGatewayClient: AlloyGatewayClient;
  private client: PluginClient;

  constructor(directory: string, AlloyGatewayClient: AlloyGatewayClient, client: PluginClient) {
    this.projectRoot = directory;
    this.AlloyGatewayClient = AlloyGatewayClient;
    this.client = client;
    this.pipeline = new SequentialPipeline(directory, AlloyGatewayClient);
      ...
      pipeline_start: tool({
        ...
        args: {
          ...
          modelOverride: tool.schema
            .string()
            .optional()
            .describe('Override model for ALL agents (e.g. "google/Alloy-claude-sonnet-4-6")'),
