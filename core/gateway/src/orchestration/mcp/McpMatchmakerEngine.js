...
exports.McpMatchmakerEngine = void 0;
...
class McpMatchmakerEngine {
    ...
    async inferDomainFromFiles() {
        try {
            ...
            if (deps['pytorch'] || deps['transformers'] || deps['tensor'])
                return 'AI';
            if (deps['react'] || deps['next'] || deps['vue'])
                return 'WEB';
            if (deps['pandas'] || deps['d3'])
                return 'DATA';
            return 'SYSTEM';
        }
        catch (_e) {
    ...
    async requestSenateApproval(mcp) {
...
exports.McpMatchmakerEngine = McpMatchmakerEngine;
//# sourceMappingURL=McpMatchmakerEngine.js.map