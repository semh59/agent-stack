...
const FORBIDDEN_ENCODED_EXECUTION = [
    {
        pattern: /\bexecute_encoded\s*\(/i,
        reason: 'Encoded execution helpers are blocked',
    },
    {
        pattern: /\b(?:eval|Function|exec|execSync|spawn|spawnSync)\s*\([^)]*(?:Buffer\.from\([^)]*base64|atob\s*\()/i,
...
class TerminalExecutor {
    ...
    async run(command, options = {}) {
        ...
        return new Promise((resolve) => {
            ...
            const child = (0, node_child_process_1.spawn)(spawnExe, spawnArgs, {
                cwd,
                timeout,
                shell: false,
            ...
            child.stdout?.on('data', (data) => {
                ...
                stdout += chunk;
                event_bus_1.eventBus.publish('terminal:data', { role: 'system', type: 'stdout', chunk, command });
            });
            child.stderr?.on('data', (data) => {
                const chunk = data.toString();
                stderr += chunk;
                event_bus_1.eventBus.publish('terminal:data', { role: 'system', type: 'stderr', chunk, command });
            });
            ...
            child.on('close', (code) => {
                ...
                event_bus_1.eventBus.publish('terminal:result', result);
                resolve(result);
            });
        });
    }
     ...
     */
    async runWithHealing(command, onFailure) {
    ...
    validateCommand(command, options) {
        ...
        const isAllowed = ALLOWLIST_PREFIXES.some((prefix) => firstWord === prefix || firstWord.endsWith(`/${prefix}`) || firstWord.endsWith(`\\${prefix}`));
        if (!isAllowed) {
