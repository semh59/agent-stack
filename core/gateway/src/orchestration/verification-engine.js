...
class VerificationEngine {
    ...
    async verify(agent, output) {
        ...
        const fullStdoutMap = new Map();
        const fullStderrMap = new Map();
     ...
     */
    static checkHaltConditions(output, commandResults, conditions) {
        const lowerOutput = output.toLowerCase();
        ...
        for (const condition of conditions) {
            ...
            if (lowerCondition.includes('secret') || lowerCondition.includes('api key')) {
                ...
                const hasNegativeMatch = negativePatterns.some(p => p.test(combined));
                if (!hasNegativeMatch && (combined.includes('secret') && combined.includes('found') ||
