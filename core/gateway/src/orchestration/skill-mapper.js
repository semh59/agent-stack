...
exports.getSkillMap = getSkillMap;
...
class SkillMapper {
    ...
    async loadSkillContent(skillName) {
        ...
        const skillPath = node_path_1.default.join(this.skillsDir, skillName, 'SKILL.md');
        try {
     ...
     */
    async buildEnrichedPrompt(agent, userSkillOverrides) {
        ...
        // 1. Inject Alloy Rules (Absolute Laws)
        try {
            const rulesPath = node_path_1.default.join(node_path_1.default.dirname(this.skillsDir), 'personals_rules.md');
            const rulesContent = await fs.readFile(rulesPath, 'utf-8');
            sections.push('## ALLOY AUTONOMY PROTOCOL (ABSOLUTE LAWS)');
        ...
        if (loadedSkills.length > 0) {
            console.log(`[SkillMapper] ${agent.role}: Injected ${loadedSkills.length} skills â†’ ${loadedSkills.join(', ')}`);
        }
