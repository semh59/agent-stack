...
describe("Storage Migration", () => {
  ...
  describe("migrateV2ToV3", () => {
    it("converts gemini rate limits to gemini-Alloy", () => {
  ...
  describe("loadAccounts migration integration", () => {
    ...
    it("migrates V2 storage on load and persists V3", async () => {
      ...
      const encryptedPayload = JSON.parse(saveCall[1] as string);
      const savedContent = km.decrypt(encryptedPayload) as any;
  ...
  describe("ensureGitignore", () => {
    const configDir = "/tmp/Alloy-test";
    ...
    it("creates .gitignore when file does not exist", async () => {
      ...
      expect(content).toContain("Alloy-accounts.json");
      expect(content).toContain("Alloy-signature-cache.json");
      expect(content).toContain("Alloy-logs/");
    });
  ...
  describe("ensureGitignoreSync", () => {
    const configDir = "/tmp/Alloy-test-sync";
