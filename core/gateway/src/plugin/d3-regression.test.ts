...
describe("D3 Bug #6: storage write survives crashes and sweeps orphans", () => {
  ...
  beforeEach(() => {
    ...
    originalConfigDir = process.env.Alloy_CONFIG_DIR;
    process.env.Alloy_CONFIG_DIR = tempDir;
  });

  afterEach(() => {
    if (originalConfigDir === undefined) {
      delete process.env.Alloy_CONFIG_DIR;
    } else {
      process.env.Alloy_CONFIG_DIR = originalConfigDir;
    }
    try {
