...
import { ALLOY_VERSION } from "../constants";
...
function updateFingerprintVersion(fingerprint: Fingerprint): Fingerprint {
  ...
  if (match) {
    ...
    const expectedUserAgent = `Alloy/${ALLOY_VERSION} ${platformArch}`;
...
export type BaseQuotaKey = "claude" | "gemini-Alloy" | "gemini-cli";
...
function getQuotaKey(family: ModelFamily, headerStyle: HeaderStyle, model?: string | null, endpoint?: string | null): QuotaKey {
  if (family === "claude") {
  ...
  if (model) {
    return `${base}:${model}` as QuotaKey;
  }
  if (endpoint) {
    return `${base}:${endpoint}` as QuotaKey;
  }
  return base;
}
...
function isRateLimitedForHeaderStyle(account: ManagedAccount, family: ModelFamily, headerStyle: HeaderStyle, model?: string | null, endpoint?: string | null): boolean {
  ...
  if (family === "claude") {
    ...
    if (endpoint && isRateLimitedForQuotaKey(account, `claude:${endpoint}` as QuotaKey)) return true;
    return isRateLimitedForQuotaKey(account, "claude");
  }
  ...
  if (model && endpoint) {
    ...
    if (isRateLimitedForQuotaKey(account, specificKey)) return true;
  }
  ...
  if (endpoint) {
    ...
    if (isRateLimitedForQuotaKey(account, endpointKey)) return true;
  }
...
export class AccountManager {
  ...
  switchToAccountByEmail(email: string, family?: ModelFamily): boolean {
  ...
  getNextForFamily(family: ModelFamily, model?: string | null, headerStyle: HeaderStyle = "Alloy", softQuotaThresholdPercent: number = 100, softQuotaCacheTtlMs: number = 10 * 60 * 1000): ManagedAccount | null {
   ...
   */
  hasOtherAccountWithAlloyAvailable(
