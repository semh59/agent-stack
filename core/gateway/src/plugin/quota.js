...
exports.checkAccountsQuota = checkAccountsQuota;
...
function classifyQuotaGroup(modelName, displayName) {
    ...
    const family = (0, model_resolver_1.getModelFamily)(modelName);
...
async function fetchAvailableModels(accessToken, projectId) {
    const endpoint = constants_1.ALLOY_ENDPOINT_PROD;
    const quotaUserAgent = constants_1.ALLOY_HEADERS["User-Agent"] || "Alloy/windows/amd64";
    const errors = [];
...
async function fetchGeminiCliQuota(accessToken, projectId) {
    const endpoint = constants_1.ALLOY_ENDPOINT_PROD;
    // Use Gemini CLI user-agent to get CLI quota buckets (not Alloy buckets)
...
async function checkAccountsQuota(accounts, client, providerId = constants_1.GOOGLE_GEMINI_PROVIDER_ID) {
    const results = [];
    (0, debug_1.logQuotaFetch)("start", accounts.length);
    for (const [index, account] of accounts.entries()) {
        const disabled = account.enabled === false;
        let auth = buildAuthFromAccount(account);
        try {
            if ((0, auth_1.accessTokenExpired)(auth)) {
                const refreshed = await (0, token_1.refreshAccessToken)(auth, client, providerId);
                if (!refreshed) {
                    throw new Error("Token refresh failed");
                }
                auth = refreshed;
            }
            const projectContext = await (0, project_1.ensureProjectContext)(auth);
            auth = projectContext.auth;
            const updatedAccount = applyAccountUpdates(account, auth);
            let quotaResult;
            let geminiCliQuotaResult = { models: [], error: undefined };
            ...
            const [AlloyResponse, geminiCliResponse] = await Promise.all([
                fetchAvailableModels(auth.access ?? "", projectContext.effectiveProjectId)
                    .catch((_error) => ({ models: undefined })),
                fetchGeminiCliQuota(auth.access ?? "", projectContext.effectiveProjectId),
            ]);
            // Process Alloy quota
            if (AlloyResponse.models === undefined) {
                quotaResult = {
                    groups: {},
                    modelCount: 0,
                    error: "Failed to fetch Alloy quota",
                };
            }
            else {
                quotaResult = aggregateQuota(AlloyResponse.models);
            }
            ...
            for (const [family, groupQuota] of Object.entries(quotaResult.groups)) {
                ...
                (0, debug_1.logQuotaStatus)(account.email, index, remainingPercent, family);
            }
        }
        catch (error) {
            results.push({
            ...
            (0, debug_1.logQuotaFetch)("error", undefined, `account=${account.email ?? index} error=${error instanceof Error ? error.message : String(error)}`);
        }
    }
    (0, debug_1.logQuotaFetch)("complete", accounts.length, `ok=${results.filter(r => r.status === "ok").length} errors=${results.filter(r => r.status === "error").length}`);
    return results;
}
