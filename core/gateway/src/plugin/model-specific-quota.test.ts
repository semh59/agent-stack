...
describe("Model-specific Gemini quota", () => {
  ...
  it("blocks only the specific Gemini model when markRateLimited is called with a model", () => {
    ...
    // Mark gemini-1.5-pro as rate limited on Alloy
    manager.markRateLimited(account, 60000, "gemini", "Alloy", modelPro);

    // gemini-1.5-pro should be rate limited for Alloy
    expect(manager.isRateLimitedForHeaderStyle(account, "gemini", "Alloy", modelPro)).toBe(true);

    // gemini-1.5-flash should NOT be rate limited for Alloy
    expect(manager.isRateLimitedForHeaderStyle(account, "gemini", "Alloy", modelFlash)).toBe(false);
  ...
  it("falls back to gemini-cli only for the specific model", () => {
    ...
    // Available header style for Flash should still be Alloy
    expect(manager.getAvailableHeaderStyle(account, "gemini", modelFlash)).toBe("Alloy");
  });
  ...
  it("returns null when all header styles are exhausted for the specific model on a single account", () => {
    ...
    manager2.markRateLimited(account, 60000, "gemini", "Alloy", modelPro);
  ...
  it("base family rate limit blocks all models in that family", () => {
    ...
    // Mark base gemini-Alloy as rate limited
    manager.markRateLimited(account, 60000, "gemini", "Alloy");

    // All Gemini models should now be blocked for Alloy on this account
    expect(manager.isRateLimitedForHeaderStyle(account, "gemini", "Alloy", modelPro)).toBe(true);
    expect(manager.isRateLimitedForHeaderStyle(account, "gemini", "Alloy", "gemini-1.5-flash")).toBe(true);
  });
});
