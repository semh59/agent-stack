...
exports.parseRateLimitReason = parseRateLimitReason;
exports.calculateBackoffMs = calculateBackoffMs;
exports.resolveQuotaGroup = resolveQuotaGroup;
exports.computeSoftQuotaCacheTtlMs = computeSoftQuotaCacheTtlMs;
exports.formatWaitTime = formatWaitTime;
const auth_1 = require("./auth");
const storage_1 = require("./storage");
const rotation_1 = require("./rotation");
const fingerprint_1 = require("./fingerprint");
const model_resolver_1 = require("./transform/model-resolver");
...
function updateFingerprintVersion(fingerprint) {
    const match = fingerprint.userAgent.match(/^Alloy\/[\d.]+ (.+)$/);
    if (match) {
        const platformArch = match[1];
        const expectedUserAgent = `Alloy/${constants_1.ALLOY_VERSION} ${platformArch}`;
...
function getQuotaKey(family, headerStyle, model, endpoint) {
    if (family === "claude") {
        // Claude usually has monolithic quota, but we can append model if needed.
        // For now, keep it simple but allow endpoint differentiation if provided.
        if (endpoint)
            return `claude:${endpoint}`;
        return "claude";
    }
    const base = headerStyle === "gemini-cli" ? "gemini-cli" : "gemini-Alloy";
    if (model && endpoint) {
...
function isRateLimitedForFamily(account, family, model) {
    ...
    const AlloyIsLimited = isRateLimitedForHeaderStyle(account, family, "Alloy", model);
    const cliIsLimited = isRateLimitedForHeaderStyle(account, family, "gemini-cli", model);
    return AlloyIsLimited && cliIsLimited;
}
function isRateLimitedForHeaderStyle(account, family, headerStyle, model, endpoint) {
    clearExpiredRateLimits(account);
    if (family === "claude") {
        // Check endpoint-specific claude limit if it exists
        if (endpoint && isRateLimitedForQuotaKey(account, `claude:${endpoint}`))
            return true;
        return isRateLimitedForQuotaKey(account, "claude");
    }
    // Check model-endpoint combo first (most specific)
    if (model && endpoint) {
        const specificKey = getQuotaKey(family, headerStyle, model, endpoint);
        if (isRateLimitedForQuotaKey(account, specificKey))
            return true;
    }
    // Then check model-specific quota
    if (model) {
        const modelKey = getQuotaKey(family, headerStyle, model);
        if (isRateLimitedForQuotaKey(account, modelKey))
            return true;
    }
    // Then check endpoint-specific quota (base style + endpoint)
    if (endpoint) {
        const endpointKey = getQuotaKey(family, headerStyle, null, endpoint);
        if (isRateLimitedForQuotaKey(account, endpointKey))
            return true;
    }
    // Finally check base family quota
...
function resolveQuotaGroup(family, model) {
    if (model) {
        return (0, model_resolver_1.getModelFamily)(model);
    }
...
function isOverSoftQuotaThreshold(account, family, thresholdPercent, cacheTtlMs, model) {
    ...
    if (isOverThreshold) {
        ...
        (0, debug_1.debugLogToFile)(`[SoftQuota] Skipping ${accountLabel}: ${quotaGroup} usage ${usedPercent.toFixed(1)}% >= threshold ${thresholdPercent}%` +
 ...
 *
 * Source of truth for the pool is `Alloy-accounts.json`.
 */
class AccountManager {
    ...
    static async loadFromDisk(authFallback) {
        const stored = await (0, storage_1.loadAccounts)();
        return new AccountManager(authFallback, stored);
    }
    constructor(authFallback, stored) {
        const authParts = authFallback ? (0, auth_1.parseRefreshParts)(authFallback.refresh) : null;
        ...
        if (stored && stored.accounts.length > 0) {
            ...
            this.accounts = stored.accounts
                ...
                const healthTracker = (0, rotation_1.getHealthTracker)();
                if (acc.healthScore !== undefined) {
                    healthTracker.setScore(index, acc.healthScore);
                }
                return {
                    index,
                    email: acc.email,
                    ...
                    fingerprint: acc.fingerprint
                        ...
                        : (0, fingerprint_1.generateFingerprint)(),
            ...
            if (this.accounts.length > 0) {
                ...
                this.currentAccountIndexByFamily.sambanova = clampNonNegativeInt(stored.activeIndexByFamily?.sambanova, defaultIndex) % this.accounts.length;
                this.currentAccountIndexByFamily.groq = clampNonNegativeInt(stored.activeIndexByFamily?.groq, defaultIndex) % this.accounts.length;
                this.currentAccountIndexByFamily.together = clampNonNegativeInt(stored.activeIndexByFamily?.together, defaultIndex) % this.accounts.length;
                this.currentAccountIndexByFamily.fireworks = clampNonNegativeInt(stored.activeIndexByFamily?.fireworks, defaultIndex) % this.accounts.length;
                this.currentAccountIndexByFamily.ollama = clampNonNegativeInt(stored.activeIndexByFamily?.ollama, defaultIndex) % this.accounts.length;
            }
            return;
        }
        // If we have stored accounts, check if we need to add the current auth
        if (authFallback && this.accounts.length > 0) {
            const authParts = (0, auth_1.parseRefreshParts)(authFallback.refresh);
        ...
        if (authFallback) {
            const parts = (0, auth_1.parseRefreshParts)(authFallback.refresh);
    ...
    switchToAccountByEmail(email, family) {
    ...
    getCurrentOrNextForFamily(family, model, strategy = 'sticky', headerStyle = 'Alloy', pidOffsetEnabled = false, softQuotaThresholdPercent = 100, softQuotaCacheTtlMs = 10 * 60 * 1000) {
        ...
        if (strategy === 'hybrid') {
            const healthTracker = (0, rotation_1.getHealthTracker)();
            const tokenTracker = (0, rotation_1.getTokenTracker)();
            ...
            const selectedIndex = (0, rotation_1.selectHybridAccount)(accountsWithMetrics, tokenTracker, currentIndex);
        ...
        if (pidOffsetEnabled && !this.sessionOffsetApplied[family] && this.accounts.length > 1) {
            ...
            (0, debug_1.debugLogToFile)(`[Account] Applying PID offset: pid=${process.pid} offset=${pidOffset} family=${family} index=${baseIndex}->${newIndex}`);
    ...
    getNextForFamily(family, model, headerStyle = "Alloy", softQuotaThresholdPercent = 100, softQuotaCacheTtlMs = 10 * 60 * 1000) {
    ...
    markRateLimited(account, retryAfterMs, family, headerStyle = "Alloy", model, endpoint) {
        const key = getQuotaKey(family, headerStyle, model, endpoint);
    ...
    markRateLimitedWithReason(account, family, headerStyle, model, reason, retryAfterMs, endpoint, failureTtlMs = 3600_000) {
        const now = nowMs();
    ...
    clearAllRateLimitsForFamily(family, model) {
        for (const account of this.accounts) {
            if (family === "claude") {
                ...
                const AlloyKey = getQuotaKey(family, "Alloy", model);
                const cliKey = getQuotaKey(family, "gemini-cli", model);
                delete account.rateLimitResetTimes[AlloyKey];
    ...
    isRateLimitedForHeaderStyle(account, family, headerStyle, model, endpoint) {
        return isRateLimitedForHeaderStyle(account, family, headerStyle, model, endpoint);
    }
    getAvailableHeaderStyle(account, family, model) {
        clearExpiredRateLimits(account);
        if (family === "claude") {
            return isRateLimitedForHeaderStyle(account, family, "Alloy") ? null : "Alloy";
        }
        if (!isRateLimitedForHeaderStyle(account, family, "Alloy", model)) {
            return "Alloy";
        }
    ...
    /**
     * Check if any OTHER account has Alloy quota available for the given family/model.
     *
     * Used to determine whether to switch accounts vs fall back to gemini-cli:
     * - If true: Switch to another account (preserve Alloy priority)
     * - If false: All accounts exhausted Alloy, safe to fall back to gemini-cli
     *
     ...
     * @returns true if any other enabled, non-cooling-down account has Alloy available
     */
    hasOtherAccountWithAlloyAvailable(currentAccountIndex, family, model) {
        ...
        return this.accounts.some(acc => {
            ...
            clearExpiredRateLimits(acc);
            // Check if Alloy is available for this account
            return !isRateLimitedForHeaderStyle(acc, family, "Alloy", model);
        });
    }
    ...
    toAuthDetails(account) {
        return {
            type: "oauth",
            refresh: (0, auth_1.formatRefreshParts)(account.parts),
    ...
    getMinWaitTimeForFamily(family, model, headerStyle, strict) {
        ...
        for (const a of this.accounts) {
            if (family === "claude") {
                ...
                const AlloyKey = getQuotaKey(family, "Alloy", model);
                const cliKey = getQuotaKey(family, "gemini-cli", model);
                const t1 = a.rateLimitResetTimes[AlloyKey];
        ...
        try {
            ...
            const storage = {
                ...
                accounts: this.accounts.map((a) => ({
                    ...
                    healthScore: (0, rotation_1.getHealthTracker)().getScore(a.index),
                })),
            ...
            await (0, storage_1.saveAccounts)(storage, true);
        }
        catch (err) {
    ...
    regenerateAccountFingerprint(accountIndex) {
        ...
        if (account.fingerprint) {
            ...
            if (account.fingerprintHistory.length > fingerprint_1.MAX_FINGERPRINT_HISTORY) {
                account.fingerprintHistory = account.fingerprintHistory.slice(0, fingerprint_1.MAX_FINGERPRINT_HISTORY);
            }
        }
        // Generate and assign new fingerprint
        account.fingerprint = (0, fingerprint_1.generateFingerprint)();
...
function getTimeUntilSoftQuotaReset(account, family, thresholdPercent, cacheTtlMs, model) {
    ...
    const quotaGroup = resolveQuotaGroup(family, model);
    const groupData = account.cachedQuota[quotaGroup];
    if (!groupData || !groupData.resetTime)
        return null;
    const resetMs = parseIsoResetTime(groupData.resetTime);
    if (resetMs === null)
        return null;
    const waitTime = resetMs - nowMs();
    return waitTime < 0 ? null : waitTime;
}
function parseIsoResetTime(resetTime) {
    try {
