...
function getConfigDir(): string {
  ...
  if (platform === "win32") {
    return join(process.env.APPDATA || join(homedir(), "AppData", "Roaming"), "Alloy");
  }
...
function getCacheFilePath(): string {
  return join(getConfigDir(), "Alloy-signature-cache.json");
}
...
export class SignatureCache {
  ...
  private saveToDisk(): boolean {
    try {
      ...
      const tmpPath = join(tmpdir(), `Alloy-cache-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`);
