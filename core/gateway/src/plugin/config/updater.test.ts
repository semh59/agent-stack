...
import { updateAlloyConfig } from "./updater";
import { Alloy_MODEL_DEFINITIONS } from "./models";

describe("updateAlloyConfig", () => {
  let tempDir: string;
  let configPath: string;

  beforeEach(() => {
    // Create a temporary directory for each test
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "Alloy-test-"));
    configPath = path.join(tempDir, "Alloy.json");
  });
  ...
  test("creates new config with default structure when file does not exist", async () => {
    const result = await updateAlloyConfig({ configPath });
    ...
    expect(writtenConfig.$schema).toBe("https://Alloy.ai/config.json");
    expect(writtenConfig.plugin).toContain("alloy-ai@latest");
  ...
  test("replaces existing google models with plugin models", async () => {
    ...
    expect(writtenConfig.provider.google.models["Alloy-gemini-3-pro"]).toBeDefined();
    expect(writtenConfig.provider.google.models["Alloy-claude-sonnet-4-5"]).toBeDefined();
  });
  ...
  test("preserves $schema and other top-level config keys", async () => {
    const existingConfig = {
      $schema: "https://Alloy.ai/config.json",
      plugin: ["alloy-ai@latest", "other-plugin"],
      theme: "dark",
  ...
  test("does not duplicate plugin if different version present", async () => {
    const existingConfig = {
      plugin: ["alloy-ai@beta", "other-plugin"],
      provider: {},
    };
    fs.writeFileSync(configPath, JSON.stringify(existingConfig));
    ...
    expect(writtenConfig.plugin).toContain("alloy-ai@beta");
  });
  ...
  test("includes all model definitions from Alloy_MODEL_DEFINITIONS", async () => {
    const result = await updateAlloyConfig({ configPath });
    ...
    // Verify all models from Alloy_MODEL_DEFINITIONS are included
    for (const modelKey of Object.keys(Alloy_MODEL_DEFINITIONS)) {
      expect(models[modelKey]).toBeDefined();
    }
  });

  test("creates parent directory if it does not exist", async () => {
    const nestedPath = path.join(tempDir, "nested", "dir", "Alloy.json");

    const result = await updateAlloyConfig({ configPath: nestedPath });
