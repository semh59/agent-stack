...
exports.analyzeConversationState = analyzeConversationState;
exports.closeToolLoopForThinking = closeToolLoopForThinking;
exports.needsThinkingRecovery = needsThinkingRecovery;
exports.looksLikeCompactedThinkingTurn = looksLikeCompactedThinkingTurn;
exports.hasPossibleCompactedThinking = hasPossibleCompactedThinking;
// ============================================================================
// TYPES
// ============================================================================
...
function messageHasThinking(msg) {
    ...
    if (Array.isArray(m.content)) {
        return m.content.some((block) => block?.type === "thinking" || block?.type === "reasoning" || block?.type === "redacted_thinking");
    }
    return false;
}
/**
...
function messageHasToolCalls(msg) {
    ...
    if (Array.isArray(m.content)) {
        return m.content.some((block) => block?.type === "tool_use" || block?.type === "tool_call");
    }
    return false;
}
// ============================================================================
...
function stripAllThinkingBlocks(contents) {
    return contents.map((content) => {
        ...
        if (Array.isArray(content.content)) {
            const filteredContent = content.content.filter((block) => block?.type !== "thinking" && block?.type !== "reasoning" && block?.type !== "redacted_thinking");
...
function closeToolLoopForThinking(contents) {
    ...
    const syntheticUser = {
        role: "user",
        parts: [{ text: constants_1.THINKING_RECOVERY_RESUME_TEXT }],
    };
 ...
 */
function looksLikeCompactedThinkingTurn(msg) {
    const m = msg;
    if (!m || typeof m !== "object")
        return false;
    const parts = m.parts || [];
 ...
 */
function hasPossibleCompactedThinking(contents, turnStartIdx) {
