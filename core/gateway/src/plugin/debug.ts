...
export const DEBUG_MESSAGE_PREFIX = "[alloy-ai debug]";

/**
 * Simple email masking helper for debug strings.
 */
function maskDebugLabel(label: string): string {
  if (label.includes('@')) {
...
function getConfigDir(): string {
  ...
  if (platform === "win32") {
    return join(env.APPDATA || join(homedir(), "AppData", "Roaming"), "Alloy");
  }
...
function getLogsDir(customLogDir?: string): string {
  const logsDir = customLogDir || join(getConfigDir(), "Alloy-logs");
...
function createLogFilePath(customLogDir?: string): string {
  ...
  return join(getLogsDir(customLogDir), `Alloy-debug-${timestamp}.log`);
}
 ...
 */
export function initializeDebug(config: AlloyGatewayConfig): void {
  // Config takes precedence, but env var can force enable for debugging
  const envDebugFlag = env.Alloy_ALLOY_DEBUG ?? "";
...
export interface AlloyDebugContext {
  id: string;
  streaming: boolean;
  startedAt: number;
}

interface AlloyDebugRequestMeta {
  originalUrl: string;
  resolvedUrl: string;
  method?: string;
  headers?: HeadersInit;
...
interface AlloyDebugResponseMeta {
  body?: string;
  note?: string;
  error?: unknown;
 ...
 */
export function startAlloyDebugRequest(meta: AlloyDebugRequestMeta): AlloyDebugContext | null {
 ...
 */
export function logAlloyDebugResponse(
  context: AlloyDebugContext | null | undefined,
  response: Response,
  meta: AlloyDebugResponseMeta = {},
): void {
  const state = getDebugState();
  ...
  logDebug(
    `[Alloy Debug ${context.id}] Response ${response.status} ${response.statusText} (${durationMs}ms)`,
  );
  logDebug(
    `[Alloy Debug ${context.id}] Response Headers: ${JSON.stringify(
  ...
  if (meta.body) {
    logDebug(
      `[Alloy Debug ${context.id}] Response Body Preview: ${truncateForLog(meta.body)}`,
    );
  }
}
