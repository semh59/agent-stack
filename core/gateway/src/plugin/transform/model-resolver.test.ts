...
describe("resolveModelWithTier", () => {
  describe("Gemini 3 flash models (Issue #109)", () => {
    it("Alloy-gemini-3-flash gets default thinkingLevel 'low'", () => {
      const result = resolveModelWithTier("Alloy-gemini-3-flash");
      expect(result.actualModel).toBe("gemini-3-flash");
      expect(result.thinkingLevel).toBe("low");
      expect(result.quotaPreference).toBe("Alloy");
    });
    ...
    it("gemini-3-flash-preview gets default thinkingLevel 'low' with Alloy quota", () => {
      const result = resolveModelWithTier("gemini-3-flash-preview");
      expect(result.actualModel).toBe("gemini-3-flash-preview");
      expect(result.thinkingLevel).toBe("low");
      // All Gemini models now default to Alloy
      expect(result.quotaPreference).toBe("Alloy");
    });
  });

  describe("Gemini 3 preview models (Issue #115)", () => {
    it("gemini-3-pro-preview gets default thinkingLevel 'low' with Alloy quota", () => {
  ...
  describe("All Gemini models default to Alloy quota", () => {
    it("gemini-2.5-flash defaults to Alloy", () => {
      const result = resolveModelWithTier("gemini-2.5-flash");
      expect(result.quotaPreference).toBe("Alloy");
    });

    it("gemini-2.5-pro defaults to Alloy", () => {
      const result = resolveModelWithTier("gemini-2.5-pro");
      expect(result.quotaPreference).toBe("Alloy");
    });

    it("gemini-2.0-flash defaults to Alloy", () => {
  ...
  describe("cli_first quota preference", () => {
    ...
    it("keeps Alloy when Alloy prefix is explicit", () => {
      const result = resolveModelWithTier("Alloy-gemini-3-flash", { cli_first: true });
      expect(result.quotaPreference).toBe("Alloy");
      expect(result.explicitQuota).toBe(true);
    });

    it("keeps Alloy for Claude models when cli_first is true", () => {
      const result = resolveModelWithTier("claude-sonnet-4-5-thinking", { cli_first: true });
      expect(result.quotaPreference).toBe("Alloy");
    });

    it("keeps Alloy for image models when cli_first is true", () => {
      const result = resolveModelWithTier("gemini-3-pro-image", { cli_first: true });
      expect(result.quotaPreference).toBe("Alloy");
      expect(result.explicitQuota).toBe(true);
    });

    it("defaults to Alloy when cli_first is false", () => {
      const result = resolveModelWithTier("gemini-3-flash", { cli_first: false });
      expect(result.quotaPreference).toBe("Alloy");
    });
  });

  describe("Alloy Gemini 3 with tier suffix", () => {
    it("Alloy-gemini-3-pro-low gets thinkingLevel from tier", () => {
      const result = resolveModelWithTier("Alloy-gemini-3-pro-low");
      expect(result.actualModel).toBe("gemini-3-pro-low");
      expect(result.thinkingLevel).toBe("low");
      expect(result.quotaPreference).toBe("Alloy");
    });

    it("Alloy-gemini-3-pro-high gets thinkingLevel from tier", () => {
      const result = resolveModelWithTier("Alloy-gemini-3-pro-high");
      expect(result.actualModel).toBe("gemini-3-pro-high");
      expect(result.thinkingLevel).toBe("high");
      expect(result.quotaPreference).toBe("Alloy");
    });

    it("Alloy-gemini-3-flash-medium gets thinkingLevel from tier", () => {
      const result = resolveModelWithTier("Alloy-gemini-3-flash-medium");
      expect(result.actualModel).toBe("gemini-3-flash");
  ...
  describe("Claude thinking models default budget", () => {
    it("Alloy-claude-sonnet-4-5-thinking gets default max budget (32768)", () => {
      const result = resolveModelWithTier("Alloy-claude-sonnet-4-5-thinking");
    ...
    it("Alloy-claude-opus-4-5-thinking gets default max budget (32768)", () => {
      const result = resolveModelWithTier("Alloy-claude-opus-4-5-thinking");
  ...
  describe("Image models", () => {
    it("marks Alloy-gemini-3-pro-image as explicit quota", () => {
      const result = resolveModelWithTier("Alloy-gemini-3-pro-image");
...
describe("resolveModelWithVariant", () => {
  ...
  describe("with variant config", () => {
    it("overrides tier budget for Claude models", () => {
      const result = resolveModelWithVariant("Alloy-claude-sonnet-4-5-thinking", {
    ...
    it("maps budget to thinkingLevel for Gemini 3 - low", () => {
      const result = resolveModelWithVariant("Alloy-gemini-3-pro", {
        thinkingBudget: 8000,
      });
      expect(result.actualModel).toBe("gemini-3-pro-low");
      expect(result.thinkingLevel).toBe("low");
    ...
    it("maps budget to thinkingLevel for Gemini 3 Flash - medium (no tier suffix)", () => {
      const result = resolveModelWithVariant("Alloy-gemini-3-flash", {
...
describe("Issue #103: resolveModelForHeaderStyle", () => {
  describe("quota fallback from gemini-cli to Alloy", () => {
    it("transforms gemini-3-flash-preview to gemini-3-flash for Alloy", () => {
      const result = resolveModelForHeaderStyle("gemini-3-flash-preview", "Alloy");
      expect(result.actualModel).toBe("gemini-3-flash");
      expect(result.quotaPreference).toBe("Alloy");
    });

    it("transforms gemini-3-pro-preview to gemini-3-pro-low for Alloy", () => {
      const result = resolveModelForHeaderStyle("gemini-3-pro-preview", "Alloy");
      expect(result.actualModel).toBe("gemini-3-pro-low");
      expect(result.quotaPreference).toBe("Alloy");
    });
  });

  describe("quota fallback from Alloy to gemini-cli", () => {
  ...
  describe("no transformation needed", () => {
    it("keeps gemini-2.5-flash unchanged for both header styles", () => {
      const Alloy = resolveModelForHeaderStyle("gemini-2.5-flash", "Alloy");
      const cli = resolveModelForHeaderStyle("gemini-2.5-flash", "gemini-cli");
      expect(Alloy.actualModel).toBe("gemini-2.5-flash");
      expect(cli.actualModel).toBe("gemini-2.5-flash");
    });

    it("keeps claude models unchanged (Alloy only)", () => {
      const result = resolveModelForHeaderStyle("claude-sonnet-4-5-thinking", "Alloy");
      expect(result.actualModel).toBe("claude-sonnet-4-5-thinking");
    });
  });
});
