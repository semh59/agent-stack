...
exports.detectToolIdMismatches = detectToolIdMismatches;
exports.findOrphanedToolUseIds = findOrphanedToolUseIds;
exports.fixClaudeToolPairing = fixClaudeToolPairing;
exports.validateAndFixClaudeToolPairing = validateAndFixClaudeToolPairing;
exports.injectParameterSignatures = injectParameterSignatures;
exports.injectToolHardeningInstruction = injectToolHardeningInstruction;
exports.assignToolIdsToContents = assignToolIdsToContents;
exports.matchResponseIdsToContents = matchResponseIdsToContents;
exports.applyToolPairingFixes = applyToolPairingFixes;
exports.createSyntheticErrorResponse = createSyntheticErrorResponse;
const logger_1 = require("../logger");
const request_helpers_1 = require("../request-helpers");
const log = (0, logger_1.createLogger)("tool-hardening");
function detectToolIdMismatches(contents) {
    const expectedIds = [];
    const foundIds = [];
...
function fixClaudeToolPairing(messages) {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (orphansForMsg && orphansForMsg.length > 0) {
            ...
            const nextContent = (nextMsg?.content || nextMsg?.parts);
            if (nextMsg && nextMsg.role === "user" && Array.isArray(nextContent)) {
                ...
                if (nextMsg.content) {
                    ...
                    nextMsg.parts = [...placeholders, ...nextMsg.parts];
                }
            }
            else {
...
function removeOrphanedToolUse(messages, orphanIds) {
    return messages
        ...
        if (msg.role === "assistant" && Array.isArray(content)) {
            const filteredContent = content.filter((block) => block.type !== "tool_use" || !orphanIds.has(block.id));
            return {
                ...msg,
                parts: filteredContent,
            };
        }
        return msg;
    })
...
function applyToolPairingFixes(payload, isClaude) {
    ...
    if (Array.isArray(payload.contents)) {
        ...
        payload.contents = (0, request_helpers_1.fixToolResponseGrouping)(contentsWithMatchedIds);
        contentsFixed = true;
...
//# sourceMappingURL=tool-hardening.js.map