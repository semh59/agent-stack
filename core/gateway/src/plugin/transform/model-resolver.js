...
exports.MODEL_FALLBACKS = exports.MODEL_ALIASES = exports.GEMINI_3_THINKING_LEVELS = exports.THINKING_TIER_BUDGETS = void 0;
exports.resolveModelWithTier = resolveModelWithTier;
exports.getModelFamily = getModelFamily;
exports.resolveModelForHeaderStyle = resolveModelForHeaderStyle;
exports.resolveModelWithVariant = resolveModelWithVariant;
/**
 ...
 */
exports.GEMINI_3_THINKING_LEVELS = ["minimal", "low", "medium", "high"];
/**
...
exports.MODEL_ALIASES = {
    ...
    "gemini-3.1-pro-high": "gemini-3-pro",
    ...
    // Image generation models - only gemini-3-pro-image is available via Alloy API
    // Note: gemini-2.5-flash-image (Nano Banana) is NOT supported by Alloy - only Google AI API
    // Reference: Alloy-Manager/src-tauri/src/proxy/common/model_mapping.rs
};
/**
 * Model fallbacks when primary model is unavailable.
 * NOTE: Image models should NOT fall back to non-image models!
 */
exports.MODEL_FALLBACKS = {
// No fallbacks for image models - they must stay as image models
};
const TIER_REGEX = /-(minimal|low|medium|high)$/;
const QUOTA_PREFIX_REGEX = /^Alloy-/i;
// ALLOY_ONLY_MODELS removed - all models now default to Alloy
/**
 * Image generation models - always route to Alloy.
 * These models don't support thinking and require imageConfig.
 */
const IMAGE_GENERATION_MODELS = /image|imagen/i;
// Legacy LEGACY_ALLOY_GEMINI3 regex removed - all Gemini models now default to Alloy
/**
 ...
 *
 * Quota routing:
 * - Default to Alloy quota unless cli_first is enabled for Gemini models
 * - Fallback to Gemini CLI happens at account rotation level when Alloy is exhausted
 * - "Alloy-" prefix marks explicit quota (no fallback allowed)
 * - Claude and image models always use Alloy
 *
 * Examples:
 * - "gemini-2.5-flash" → { quotaPreference: "Alloy" }
 * - "gemini-3-pro-preview" → { quotaPreference: "Alloy" }
 * - "Alloy-gemini-3-pro-high" → { quotaPreference: "Alloy", explicitQuota: true }
 * - "claude-sonnet-4-5-thinking-medium" → { quotaPreference: "Alloy" }
 *
...
function resolveModelWithTier(requestedModel, options = {}) {
    const isAlloy = QUOTA_PREFIX_REGEX.test(requestedModel);
    ...
    // All models default to Alloy quota unless cli_first is enabled
    // Fallback to gemini-cli happens at the account rotation level when Alloy is exhausted
    const preferGeminiCli = options.cli_first === true && !isAlloy && !isImageModel && !isClaudeModel;
    const quotaPreference = preferGeminiCli ? "gemini-cli" : "Alloy";
    const explicitQuota = isAlloy || isImageModel;
    const isGemini3 = modelWithoutQuota.toLowerCase().startsWith("gemini-3");
    const skipAlias = isAlloy && isGemini3;
    // For Alloy Gemini 3 Pro models without explicit tier, append default tier (-low)
    // Alloy API: gemini-3-pro requires tier suffix (gemini-3-pro-low/high)
    ...
    let AlloyModel = modelWithoutQuota;
    if (skipAlias) {
        if (isGemini3Pro && !tier && !isImageModel) {
            AlloyModel = `${modelWithoutQuota}-low`;
        }
    ...
    const actualModel = skipAlias
        ? AlloyModel
        : exports.MODEL_ALIASES[modelWithoutQuota] || exports.MODEL_ALIASES[baseName] || baseName;
    const resolvedModel = exports.MODEL_FALLBACKS[actualModel] || actualModel;
    ...
    if (!tier) {
        ...
        if (isClaudeThinking) {
            return {
                actualModel: resolvedModel,
                thinkingBudget: exports.THINKING_TIER_BUDGETS.claude.high,
                isThinkingModel: true,
                quotaPreference,
                explicitQuota,
            };
    ...
    const budgets = exports.THINKING_TIER_BUDGETS[budgetFamily];
 ...
 * Transforms model names when switching between gemini-cli and Alloy quotas.
 *
 * Issue #103: When quota fallback occurs, model names need to be transformed:
 * - gemini-3-flash-preview (gemini-cli) → gemini-3-flash (Alloy)
 * - gemini-3-pro-preview (gemini-cli) → gemini-3-pro-low (Alloy)
 * - gemini-3-flash (Alloy) → gemini-3-flash-preview (gemini-cli)
 */
function resolveModelForHeaderStyle(requestedModel, headerStyle) {
    ...
    if (headerStyle === "Alloy") {
        ...
        const prefixedModel = `Alloy-${transformedModel}`;
 ...
 */
function resolveModelWithVariant(requestedModel, variantConfig) {
