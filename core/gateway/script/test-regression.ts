...
const GEMINI_FLASH = "google/alloy-gemini-3-flash";
const GEMINI_FLASH_CLI_QUOTA = "google/gemini-2.5-flash";
const CLAUDE_SONNET = "google/alloy-claude-sonnet-4-5-thinking-low";
const CLAUDE_OPUS = "google/alloy-claude-opus-4-5-thinking-low";
...
const HEAVY_TESTS: MultiTurnTest[] = [
  ...
  {
    name: "Alloy-tools-comprehensive",
    model: CLAUDE_SONNET,
    category: "multi-tool",
    suite: "heavy",
    turns: [
...
const CONCURRENT_TESTS: ConcurrentTest[] = [
  ...
  {
    name: "concurrent-10-alloy-heavy",
    category: "concurrency",
    suite: "heavy",
...
async function deleteSession(sessionId: string): Promise<void> {
  return new Promise((resolve) => {
    ...
    const proc = spawn("Alloy", ["session", "delete", sessionId, "--force"], {
      stdio: ["ignore", "pipe", "pipe"],
      timeout: 10000,
      cwd: process.cwd(),
      shell: isWin,
...
async function runMultiTurnTest(test: MultiTurnTest): Promise<TestResult> {
  ...
  for (let index = 0; index < test.turns.length; index++) {
    ...
    if (result.code !== 0 && result.code !== null) {
      process.stdout.write("\r" + " ".repeat(50) + "\r");
      let msg = `Turn ${index + 1}: Failed with exit code ${result.code}`;
      
      const isTimeout = Date.now() - turnStart >= test.timeout - 1000;
      if (isTimeout) {
        msg = `Turn ${index + 1}: Timeout after ${test.timeout}ms`;
      } else if (result.stderr.toLowerCase().includes("enoent")) {
        msg = `Turn ${index + 1}: ENOENT (Executable not found)\n    Stderr: ${result.stderr}`;
...
function showHelp(): void {
  console.log(`
Multi-Turn Regression Test Suite for Alloy AI Plugin
...
Tests:
  ...
  Heavy (stress, endurance):
    ...
    - Alloy-tools-comprehensive (6 turns, all tools)
