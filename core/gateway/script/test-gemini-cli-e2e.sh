...
test_model() {
  ...
  # Run Alloy with a simple prompt
  timeout 60 Alloy run -m "$model" \
...
# Test 5: Cross-model session (gemini-cli → alloy)
echo "Test 5: Cross-model session (gemini-cli → alloy-gemini)"
log_info "Step 1: Start with gemini-2.5-flash..."
timeout 60 Alloy run -m google/gemini-2.5-flash \
  "Say: SESSION_START" \
  2>&1 > /tmp/gemini-cli-e2e-cross-s1.log || true

# Get session ID
sleep 1
SID=$(Alloy session list 2>/dev/null | grep -oP 'ses_[a-zA-Z0-9]+' | head -1 || true)

if [ -z "$SID" ]; then
  log_fail "Test 5 - No session ID created"
else
  log_info "Session: $SID"
  log_info "Step 2: Switch to alloy-gemini-3-flash..."
  timeout 60 Alloy run -s "$SID" -m google/alloy-gemini-3-flash \
  ...
  if check_auth_error /tmp/gemini-cli-e2e-cross-s2.log; then
    ...
    log_pass "Test 5 - Cross-model session (gemini-cli → alloy)"
  fi
fi
echo ""

# Test 6: Reverse cross-model (alloy → gemini-cli)
echo "Test 6: Cross-model session (alloy → gemini-cli)"
log_info "Step 1: Start with alloy-gemini-3-pro-low..."
timeout 60 Alloy run -m google/alloy-gemini-3-pro-low \
  "Say: ALLOY_START" \
...
if [ -z "$SID" ]; then
  ...
  timeout 60 Alloy run -s "$SID" -m google/gemini-2.5-pro \
  ...
  if check_auth_error /tmp/gemini-cli-e2e-reverse-s2.log; then
    ...
    log_pass "Test 6 - Cross-model session (alloy → gemini-cli)"
  fi
fi
echo ""
