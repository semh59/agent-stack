...
import { AlloyGatewayConfigSchema } from "../src/plugin/config/schema.js";

const __dirname = dirname(fileURLToPath(import.meta.url));
const outputPath = join(__dirname, "../assets/alloy.schema.json");

// Use zod v4's built-in toJSONSchema method
const rawSchema = AlloyGatewayConfigSchema.toJSONSchema({
  unrepresentable: "any",
...
const envVarDescriptions: Record<string, string> = {
  quiet_mode:
    "Suppress most toast notifications (rate limit, account switching). Recovery toasts always shown. Env: OPENCODE_ALLOY_QUIET=1",
  debug:
    "Enable debug logging to file. Env: OPENCODE_ALLOY_DEBUG=1 (or =2 for verbose)",
  log_dir:
    "Custom directory for debug logs. Env: OPENCODE_ALLOY_LOG_DIR=/path/to/logs",
  keep_thinking:
    "Preserve thinking blocks for Claude models using signature caching. May cause signature errors. Env: OPENCODE_ALLOY_KEEP_THINKING=1",
  session_recovery:
    "Enable automatic session recovery from tool_result_missing errors. Env: OPENCODE_ALLOY_SESSION_RECOVERY=1",
  auto_resume:
    "Automatically send resume prompt after successful recovery. Env: OPENCODE_ALLOY_AUTO_RESUME=1",
  resume_text:
    "Custom text to send when auto-resuming after recovery. Env: OPENCODE_ALLOY_RESUME_TEXT=continue",
  empty_response_max_attempts:
    "Maximum retry attempts when Alloy AI returns an empty response (no candidates).",
  ...
  auto_update: "Enable automatic plugin updates. Env: OPENCODE_ALLOY_AUTO_UPDATE=1",
};
...
if (definitions?.AlloyGatewayConfig) {
  addDescriptions(definitions.AlloyGatewayConfig);
} else {
