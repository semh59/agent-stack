...
#
# Models tested:
# 1. Gemini (google/alloy-gemini-3-pro-low, gemini-3-flash)
# 2. Claude via Anthropic (anthropic/claude-opus-4-5)
# 3. Claude via Google (google/alloy-claude-*-thinking-*)
...
get_session_id() {
  sleep 1
  Alloy session list 2>/dev/null | grep -oP 'ses_[a-zA-Z0-9]+' | head -1 || true
}
...
Alloy run -m google/alloy-gemini-3-pro-low \
...
if [ -z "$SID" ]; then
  ...
  Alloy run -s "$SID" -m anthropic/claude-opus-4-5 \
...
if [ -z "$SID" ]; then
  ...
  Alloy run -s "$SID" -m google/alloy-claude-opus-4-5-thinking-low \
...
if [ -z "$SID" ]; then
  ...
  Alloy run -s "$SID" -m openai/gpt-5.2-medium \
...
Alloy run -m anthropic/claude-opus-4-5 \
...
if [ -z "$SID" ]; then
  ...
  Alloy run -s "$SID" -m google/alloy-gemini-3-pro-low \
...
Alloy run -m openai/gpt-5.2-medium \
...
if grep -qi "api.*key\|unauthorized\|authentication" /tmp/e2e-t5-s1.log; then
  ...
  if [ -z "$SID" ]; then
    ...
    Alloy run -s "$SID" -m google/alloy-claude-sonnet-4-5-thinking-low \
...
if [ -z "$SID" ]; then
  ...
  Alloy run -s "$SID" -m google/alloy-gemini-3-flash \
...
Alloy run -m google/alloy-claude-sonnet-4-5-thinking-low \
...
Alloy run -m google/alloy-gemini-3-flash \
