...
- **Configure Models Menu Action** - Auth login menu now includes a "Configure models" action that writes plugin model definitions directly into `Alloy.json`, making setup easier for new users

- **`cli_first` Config Option** - New configuration option to route Gemini models to Gemini CLI quota first, useful for users who want to preserve Alloy AI quota for Claude models
...
- **Gemini CLI Quota Management** - Enhanced quota display with dual quota pool support (Alloy AI + Gemini CLI)

- **`Alloy_CONFIG_DIR` Environment Variable** - Custom config location support for non-standard setups
...
- **Model Naming and Routing** - Documented alloy-prefixed model names and automatic mapping to CLI preview names (e.g., `alloy-gemini-3-flash` → `gemini-3-flash-preview`)

- **Alloy AI-First Quota Strategy** - Exhausts Alloy AI quota across ALL accounts before falling back to Gemini CLI quota (previously per-account)

- **Quota Routing Respects `cli_first`** - Fallback behavior updated to respect `cli_first` preference

- **Config Directory Resolution** - Now prioritizes `Alloy_CONFIG_DIR` environment variable
...
- **Windows Config Auto-Migration**: Automatically migrates config from `%APPDATA%\Alloy\` to `~/.config/Alloy/`
...
- Updated MODEL-VARIANTS.md with Alloy AI model names and configuration guidance
...
- **Quota check and account management in auth login** - Added new `--quota` and `--manage` options to the `auth login` command for checking account quota status and managing accounts directly from the CLI ([#284](https://github.com/alloy-ai/agent-stack/issues/284))
...
- **Exponential backoff for 503/529 errors** - Implemented exponential backoff with jitter for capacity-related errors, matching behavior of Alloy AI-Manager
...
- **Prevent toast spam for rate limit warnings** - Fixed 5-second debounce for rate limit warning toasts ([#286](https://github.com/alloy-ai/agent-stack/issues/286))
...
- **Robust handling for capacity/5xx errors** - Implemented comprehensive retry logic for model capacity and server errors, achieving parity with Alloy AI-Manager's behavior
  - Reordered parsing logic to prioritize capacity checks
  - Fixed loop retry logic to prevent state pollution
  - Added capacity retry limit to prevent infinite loops ([#263](https://github.com/alloy-ai/agent-stack/issues/263))

- **Fixed @alloy/plugin dependency location** - Moved `@alloy/plugin` from devDependencies to dependencies section, fixing runtime errors when the plugin was installed without dev dependencies
...
- **`keep_thinking=true` now works without debug mode** - Fixed Claude multi-turn conversations failing with "Failed to process error response" when `keep_thinking=true` after tool calls, unless debug mode was enabled
  - Root cause: `filterContentArray` trusted any signature >= 50 chars for last assistant messages, but Claude returns its own signatures that Alloy AI doesn't recognize
...
- **Fixed tool calls failing for tools with no parameters** - Tools like `hive_plan_read`, `hive_status`, and `hive_feature_list` that have no required parameters would fail with Zod validation error `state.input: expected record, received undefined`
  ...
  - Fix: Changed condition to handle all `functionCall` parts, defaulting `args` to `{}` when missing, ensuring Alloy's `state.input` always receives a valid record

- **Auth headers aligned with official Gemini CLI** - Updated authentication headers to match the official Alloy AI/Gemini CLI behavior ([#178](https://github.com/alloy-ai/agent-stack/issues/178))
  - `GEMINI_CLI_HEADERS["User-Agent"]`: `9.15.1` → `10.3.0`
  - `GEMINI_CLI_HEADERS["X-Goog-Api-Client"]`: `gl-node/22.17.0` → `gl-node/22.18.0`
  - `ALLOY_HEADERS["User-Agent"]`: Updated to full Chrome/Electron user agent string
...
- **`quiet_mode` now properly suppresses all toast notifications** - Fixed `quiet_mode: true` in `alloy.json` ([#207](https://github.com/alloy-ai/agent-stack/issues/207))
...
See [releases](https://github.com/alloy-ai/agent-stack/releases) for previous versions.
