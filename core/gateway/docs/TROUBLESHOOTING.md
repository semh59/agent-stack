...
Common issues and solutions for the Alloy AI Auth plugin.
...
### "This version of Alloy AI is no longer supported"
This almost always means an outdated Alloy AI `User-Agent` is still being used.

1) Stop any running Alloy processes (stale processes can overwrite your accounts file):
...
**Windows (PowerShell):**
```powershell
Stop-Process -Name "Alloy" -Force -ErrorAction SilentlyContinue
```
...
**macOS/Linux:**
```bash
rm -f ~/.config/Alloy/alloy-accounts.json
rm -rf ~/.cache/Alloy/node_modules/alloy-ai
rm -rf ~/.bun/install/cache/alloy-ai*
Alloy auth login
```

**Windows (PowerShell):**
```powershell
Remove-Item "$env:APPDATA\Alloy\alloy-accounts.json" -Force -ErrorAction SilentlyContinue
Remove-Item "$env:LOCALAPPDATA\Alloy\Cache\node_modules\alloy-ai" -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item "$env:USERPROFILE\.bun\install\cache\alloy-ai*" -Recurse -Force -ErrorAction SilentlyContinue
Alloy auth login
```
...
**Why this happens:** The plugin defaults to a predefined project ID that doesn't exist in your Google Cloud account. Alloy AI models work, but Gemini CLI models need your own project.
...
4. Add `projectId` to your account in `~/.config/Alloy/alloy-accounts.json`:
...
3. Report the specific MCP in a [GitHub issue](https://github.com/alloy-ai/agent-stack/issues)
...
2. **Alloy's request pattern** — Alloy makes more API calls than native apps (tool calls, retries, streaming, multi-turn chains), which triggers limits faster than "normal" usage
...
<details>
<summary><b>2. "Warm up" accounts in Alloy AI IDE (community tip)</b></summary>

Users have reported success with this approach:

1. Open [Alloy AI IDE](https://idx.google.com/) directly in your browser
...
- If using oh-my-Alloy, consider reducing concurrent agent spawns
- Set `max_rate_limit_wait_seconds: 0` to fail fast instead of retrying

</details>

<details>
<summary><b>4. Use Alloy AI IDE directly (single account users)</b></summary>

If you only have one account, you'll likely have a better experience using [Alloy AI IDE](https://idx.google.com/) directly instead of routing through Alloy, since Alloy's request pattern triggers limits faster.
...
1. Delete accounts file: `rm ~/.config/Alloy/alloy-accounts.json`
2. Re-authenticate: `Alloy auth login`
3. Update to latest beta: `"plugin": ["alloy-ai@beta"]`
4. Consider "warming up" the account in Alloy AI IDE first

</details>

**What to report:**

If you're seeing unusual rate limit behavior, please share in a [GitHub issue](https://github.com/alloy-ai/agent-stack/issues):
...
1. **Use a different browser** (easiest):
   Copy the URL from `Alloy auth login` and paste it into Chrome or Firefox.
...
3. **Manual callback extraction** (advanced):
   ...
   - See [issue #119](https://github.com/alloy-ai/agent-stack/issues/119) for manual auth support
...
The OAuth callback requires the browser to reach `localhost` on the machine running Alloy.
...
```json
{
  "alloy-claude-sonnet-4-5-thinking-low": { ... },
  "alloy-claude-sonnet-4-5-thinking-max": { ... }
}
```
...
v1.2.7+ uses explicit `alloy-` prefix:

| Old Name | New Name |
|----------|----------|
| `gemini-3-pro-low` | `alloy-gemini-3-pro` |
| `claude-sonnet-4-5` | `alloy-claude-sonnet-4-5` |

Old names work as fallback, but `alloy-` prefix is recommended.
...
```bash
ALLOY_GATEWAY_DEBUG=1 alloy   # Basic
ALLOY_GATEWAY_DEBUG=2 alloy   # Verbose (full request/response)
```

Logs are in `~/.config/Alloy/alloy-logs/`.
...
Open an issue on [GitHub](https://github.com/alloy-ai/agent-stack/issues).