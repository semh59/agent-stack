...
This document explains how the Alloy AI plugin works, including the request/response flow, Claude-specific handling, and session recovery.
...
```
┌─────────────────────────────────────────────────────────────────┐
│  Alloy ──▶ Plugin ──▶ Alloy AI API ──▶ Claude/Gemini      │
│     │           │              │                   │            │
│     │           │              │                   └─ Model     │
...
The plugin intercepts requests to `generativelanguage.googleapis.com`, transforms them for the Alloy AI API, and handles authentication, rate limits, and error recovery.
...
```typescript
fetch() intercepted → isGenerativeLanguageRequest() → prepareAlloy AIRequest()
```
...
Claude through Alloy AI requires:
...
3. **Dual quota (Gemini)** - Alloy AI + Gemini CLI headers
...
- `POST /api/missions` performs an active-token preflight. An email alone is not enough; the active Alloy AI token must also be valid or refreshable.
...
- `verify` can still be valid with `touchedFiles=[]` for no-file task types (for example `analysis`, `finalize`); warning logs for that path are opt-in via `ALLOY_WARN_VERIFY_NO_TOUCHED=1`.
...
Mission budgets now use Alloy AI-facing TPM/RPD quotas instead of USD cost.
...
Before switching autonomy traffic to a real Alloy AI mission path, the orchestration client supports deterministic network simulation and graceful recovery:

- **Mock latency:** internal header `x-alloy-mock-latency-ms` delays dispatch without forwarding that header upstream.
...
- **Model request timeout:** `AutonomySessionManager` enforces a local fail-fast timeout via `AbortSignal.timeout(...)`.
  ...
  - This is a local safety default, **not** an Alloy AI provider SLA guarantee.
...
- Default DB path: `~/.config/alloy/missions.db` via `xdg-basedir`.
...
| Variable | Purpose |
|----------|---------|
| `Alloy_ALLOY_DEBUG` | `1` or `2` for debug logging |
| `Alloy_ALLOY_QUIET` | Suppress toast notifications |

### Config File

Location: `~/.config/Alloy/alloy.json`
...
| Function | Purpose |
|----------|---------|
| `prepareAlloy AIRequest()` | Main request transformation |
| `transformAlloy AIResponse()` | SSE streaming, format conversion |
...
| `cleanJSONSchemaForAlloy AI()` | Schema sanitization |
...
```bash
export Alloy_ALLOY_DEBUG=2  # Verbose
```

### Log Location

`~/.config/Alloy/alloy-logs/`
...
- [ALLOY_API_SPEC.md](./ALLOY_API_SPEC.md) - API reference
