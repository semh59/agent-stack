...
This guide covers migration from pre-security-hardening builds to current `alloy-ai`.

## 1) Gateway Security Defaults

### Breaking
- Gateway now binds to `127.0.0.1` by default.
- Gateway token is mandatory (`ALLOY_GATEWAY_TOKEN` / `authToken` option).
...
2. Export token in runtime environment:
   - PowerShell: `$env:ALLOY_GATEWAY_TOKEN='...'`
   - Bash: `export ALLOY_GATEWAY_TOKEN='...'`
3. Restart gateway.
...
### Required Action
- If extra endpoints are needed, add them through `alloy.gatewayConnectOrigins`.
- Keep origin list explicit and minimal.

## 5) Script Names and Branding

### Breaking
- Canonical name is `alloy-ai`.
