...
This document provides a deep technical reference for the Alloy AI OAuth authentication system — covering token lifecycle, multi-account management, load balancing, quota protection, and gateway authentication.
...
|---------|---------|---------|
| `@openauthjs/openauth/pkce` | `alloy/oauth.ts` | PKCE code generation |
...
| `src/alloy/oauth.ts` | ~200 | — | OAuth token exchange (authorize → exchange), PKCE state management |
...
```
┌────────┐     ┌──────────┐     ┌──────────────┐     ┌─────────────┐
│ Client  │     │ Browser  │     │ Google OAuth2│     │ Alloy AI │
│ (Plugin │     │          │     │ Server       │     │ API         │
└───┬─────┘     └────┬─────┘     └──────┬───────┘     └──────┬──────┘
    │                │                  │                    │
    │ 1. Generate    │                  │                    │
...
|------|-----------|--------------|
| 1 | `alloy/oauth.ts` | `PKCEStateManager` generates `verifier` + SHA256 `challenge`, stores in server-side session map (10-min TTL, one-time use) |
...
| 6 | `alloy/oauth.ts` → `exchangeAlloy()` | Sends `code` + `verifier` + `client_secret` to Alloy API token endpoint |
...
|--------|---------|----------------|
| `PKCEStateManager` (class in `alloy/oauth.ts`) | Plugin + Gateway auth flow | Full stateful manager with server-side session storage, one-time use, 10-min TTL |
...
// alloy/oauth.ts — Full stateful PKCE manager
...
|----------|-------|---------|
| `ALLOY_CLIENT_ID` | Google OAuth2 client ID | Identifies the Alloy AI application |
| `ALLOY_CLIENT_SECRET` | From `ALLOY_CLIENT_SECRET` env or fallback | Client secret for token exchange |
| `ALLOY_SCOPES` | 5 scopes (see below) | OAuth permission scopes |
| `ALLOY_REDIRECT_URI` | `http://127.0.0.1:51121/oauth-callback` | Local callback URL |
| `ALLOY_ENDPOINT_FALLBACKS` | daily → autopush → prod | Ordered endpoint fallback list |
| `ALLOY_LOAD_ENDPOINTS` | prod-first ordering | Endpoint order for `loadCodeAssist` |
| `ALLOY_VERSION` | `1.15.8` | Version string for User-Agent header |
| `ALLOY_ENDPOINT` | Points to `DAILY` | Primary API endpoint (development/testing) |
...
|----------|-----|---------|
| `ALLOY_ENDPOINT_DAILY` | `https://daily-cloudcode-pa.sandbox.googleapis.com` | Daily build (primary default) |
| `ALLOY_ENDPOINT_AUTOPUSH` | `https://autopush-cloudcode-pa.sandbox.googleapis.com` | Autopush staging |
| `ALLOY_ENDPOINT_PROD` | `https://cloudcode-pa.googleapis.com` | Production |

**Fallback order:** `ALLOY_ENDPOINT_FALLBACKS = [daily, autopush, prod]`
**Load order:** `ALLOY_LOAD_ENDPOINTS = [prod, ...]` (prod-first for `loadCodeAssist`)
...
| Style | Use Case |
|-------|----------|
| `alloy` | Default Alloy AI API requests |
...
### Config File (`~/.config/Alloy/alloy.json`)
...
| **Refresh Token** | Indefinite (until revoked) | Disk (`alloy-accounts.json`) | Obtain new access tokens |
...
│                                                              │
│  ┌───────────────────────────────────────────────────────┐  │
│  │ Persistence (alloy-accounts.json)                │  │
...
interface RateLimitStateV3 {
  ...
  "gemini-alloy"?: number; // Reset timestamp for Gemini via Alloy AI
...
|------|----------|-----------|
| **Alloy AI** | Alloy AI API | Default for all requests |
| **Gemini CLI** | Gemini API directly | Automatic fallback when Alloy AI exhausted on ALL accounts |

Transition flow:

```
1. Request on Account A (Alloy AI) ──▶ 429
2. Try Account B (Alloy AI)         ──▶ 429
3. Try Account C (Alloy AI)         ──▶ 429
4. All Alloy AI exhausted
...
Machine ID (node-machine-id)
    + Application Salt (ALLOY_SALT env or 'ag-default-salt-v3')
    ────────────────────────────────────────────────
    ▼ scryptSync(machineId, salt, 32)
    Master Key (32 bytes)

Override: ALLOY_MASTER_KEY env → raw hex key (for CI/headless)
```
...
interface Fingerprint {
  ...
  userAgent: string;         // "alloy/{version} {platform}/{arch}"
...
Location: `~/.config/Alloy/alloy-accounts.json`
  ...
  "accounts": [
    {
      ...
      "fingerprint": {
        ...
        "userAgent": "alloy/1.15.8 darwin/arm64",
...
| **Default port** | Derived from `ALLOY_REDIRECT_URI` (51121) |
...
**Storage location:** `~/.config/agent/alloy-tokens.json` (encrypted via `KeyManager`)
...
interface LaunchResult {
  authorization: Alloy AIAuthorization;
  browserOpened: boolean;
}

async function launchOAuthBrowser(projectId?: string): Promise<LaunchResult>
async function generateOAuthUrl(projectId?: string): Promise<Alloy AIAuthorization>
```
...
```typescript
const DEFAULT_OAUTH_CALLBACK_PORT: number  // Parsed from ALLOY_REDIRECT_URI (51121)
...
The plugin-layer OAuth callback server, used by the CLI/Alloy integration.

### Key Differences from Gateway Server

| Aspect | Plugin Server | Gateway Server |
|--------|---------------|----------------|
| **Use case** | CLI/Alloy login | Gateway web/server auth |
...
- **HTTPS** used for all Google/Alloy AI API calls
...
| Test File | Tests |
|-----------|-------|
| `src/alloy/oauth.test.ts` | OAuth token exchange, PKCE state management |
