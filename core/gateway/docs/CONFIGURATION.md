...
Create `~/.config/Alloy/alloy.json` (or `.Alloy/alloy.json` in project root):

```json
{
  "$schema": "https://raw.githubusercontent.com/alloy-ai/agent-stack/main/assets/alloy.schema.json"
}
```
...
| `quota_fallback` | `false` | **Gemini only.** When Alloy AI exhausted on ALL accounts, fall back to Gemini CLI quota |
...
ALLOY_GATEWAY_DEBUG=1 alloy   # Basic logging
ALLOY_GATEWAY_DEBUG=2 alloy   # Verbose logging

# Via config (persistent)
{ "debug": true }
```

Logs are written to `~/.config/Alloy/alloy-logs/` (or `log_dir` if set).
...
```bash
ALLOY_GATEWAY_QUIET=1                              # quiet_mode
ALLOY_GATEWAY_DEBUG=1                              # debug (1=basic, 2=verbose)
ALLOY_GATEWAY_LOG_DIR=/path                        # log_dir
ALLOY_GATEWAY_KEEP_THINKING=1                      # keep_thinking
ALLOY_GATEWAY_ACCOUNT_SELECTION_STRATEGY=round-robin
ALLOY_GATEWAY_PID_OFFSET_ENABLED=1
```
