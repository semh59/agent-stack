...
Alloy's variant system allows you to configure thinking budget dynamically instead of defining separate models for each thinking level.

---

## How Variants Work

When you define a model with `variants`, Alloy shows variant options in the model picker. Selecting a variant passes the `providerOptions` to the plugin, which extracts the thinking configuration.
...
- `alloy-claude-sonnet-4-5-thinking-medium`
- `alloy-claude-sonnet-4-5-thinking-high`
- `alloy-gemini-3-pro-low`
- `alloy-gemini-3-pro-high`
- `gemini-3-pro-low`
...
- **Simpler config** — No need to configure both `alloy-` and `-preview` versions
...
- **Future-proof** — Works with Alloy's native variant system
