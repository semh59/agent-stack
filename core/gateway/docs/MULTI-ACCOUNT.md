...
- **Alloy AI-first for Gemini** — All Gemini requests use Alloy AI quota first, then automatically fall back to Gemini CLI when exhausted across all accounts
...
|------------|-----------|
| **Alloy AI** | Default for all requests |
| **Gemini CLI** | Automatic fallback when Alloy AI exhausted on ALL accounts |

This effectively **doubles your Gemini quota** when you have `quota_fallback` enabled.

### How Quota Fallback Works

1. Request uses Alloy AI quota on current account
2. If rate-limited, plugin checks if ANY other account has Alloy AI available
3. If yes → switch to that account (stay on Alloy AI)
...
To enable automatic fallback between pools, set in `alloy.json`:
...
For checking quotas outside Alloy (debugging, CI, etc.):
...
When running `Alloy auth login` with existing accounts:
...
Accounts are stored in `~/.config/Alloy/alloy-accounts.json`:
...
## Parallel Sessions (oh-my-Alloy)

When using oh-my-Alloy with parallel subagents, multiple processes may select the same account, causing rate limit errors.

**Solution:** Enable PID-based offset in `alloy.json`:
...
Alternatively, add more accounts via `Alloy auth login`.

---

## Account Selection Strategies

Configure in `alloy.json`:
