 ...
 *
 * Google OAuth ile giriş yapıp Alloy AI agent sistemini başlatan CLI scripti.
 *
 * Kullanım:
...
function parseArgs(): CliArgs {
  ...
  const result: CliArgs = {
    authOnly: false,
    model: "google/alloy-claude-sonnet-4-5",
    autonomy: "supervised",
...
async function agentLoop(result: HandoffResult): Promise<void> {
  ...
  // AlloyGatewayClient hazır — buradan agent sistemi devralabilir
  const { client } = result;
  ...
  try {
    ...
    if (response.ok) {
      console.log("[Agent] ✅ Alloy AI gateway bağlantısı başarılı!");
    } else {
