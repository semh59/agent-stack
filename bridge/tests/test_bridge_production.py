...
Tests for bridge.py production-mode hardening:
  - refuses to boot without AI_STACK_BRIDGE_SECRET in staging/prod
...
def _reset_bridge_module():
    """Force bridge module to be re-imported with current env."""
    for modname in list(sys.modules):
        if modname == "bridge" or modname.startswith("bridge."):
            del sys.modules[modname]
    sys.path.insert(0, str(ROOT))


def test_bridge_exits_in_production_without_secret(monkeypatch, tmp_path):
    """APP_ENV=production and no secret → sys.exit(78) EX_CONFIG."""
    monkeypatch.setenv("APP_ENV", "production")
    monkeypatch.setenv("AI_STACK_DATA_DIR", str(tmp_path))
    monkeypatch.delenv("AI_STACK_BRIDGE_SECRET", raising=False)
...
def test_bridge_generates_dev_secret_in_development(monkeypatch, tmp_path):
    """In development mode, a missing secret gets an ephemeral one (warning logged)."""
    ...
    assert bridge._BRIDGE_SECRET, "dev mode should generate a fallback secret"
    assert len(bridge._BRIDGE_SECRET) >= 32


def test_bridge_uses_env_secret_when_provided(monkeypatch, tmp_path):
    monkeypatch.setenv("APP_ENV", "production")
    monkeypatch.setenv("AI_STACK_DATA_DIR", str(tmp_path))
    monkeypatch.setenv("AI_STACK_BRIDGE_SECRET", "supersecret-value-1234567890")

    _reset_bridge_module()

    bridge = importlib.import_module("bridge")
    assert bridge._BRIDGE_SECRET == "supersecret-value-1234567890"


def test_check_auth_constant_time_compare(monkeypatch, tmp_path):
    """_check_auth uses hmac.compare_digest — verify correctness, not timing."""
    monkeypatch.setenv("APP_ENV", "development")
    monkeypatch.setenv("AI_STACK_DATA_DIR", str(tmp_path))
    monkeypatch.setenv("AI_STACK_BRIDGE_SECRET", "s3cret")
